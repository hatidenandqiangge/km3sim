...
KM3HAEnergyFlux::KM3HAEnergyFlux(char* infileParam,G4double QEmax, G4double TotCathodArea, G4double EneMin, G4double EneMax)
{
  NPartsDists=2; //is the number of distributions kept (now it is 2, for 211 (pi+) and 130 (KaonZeroLong))
  ...
  //they are for particles (-2212, -2112, 130, +-211, +-321, 2112, 2212) in this order
  //it has been fitted (flux/(energy/TeV) vs log10(energy/TeV) with a poly 4th order
  //-2212
...
G4double KM3HAEnergyFlux::Rescale(G4int idbeam, G4double energyin,G4double energydis)
{
  G4int irbin;
  //they are for particles (-2212, -2112, 130, +-211, +-321, 2112, 2212) in this order
  if(idbeam ==-2212)irbin=0;
  else if(idbeam ==-2112)irbin=1;
  else if(idbeam ==130)irbin=2;
  else if(idbeam == 211 || idbeam == -211)irbin=3;
  else if(idbeam == 321 || idbeam == -321)irbin=4;
  else if(idbeam ==2112)irbin=5;
  else if(idbeam ==2212)irbin=6;
  G4double eneinlog=log10(energyin/TeV);
  G4double enedislog=log10(energydis/TeV);
  G4double ydis=ParticleEnergyScaleFactors[irbin][0]+enedislog*(
                ParticleEnergyScaleFactors[irbin][1]+enedislog*(
                ParticleEnergyScaleFactors[irbin][2]+enedislog*(
                ParticleEnergyScaleFactors[irbin][3]+enedislog*(
                ParticleEnergyScaleFactors[irbin][4]))));
  G4double yin= ParticleEnergyScaleFactors[irbin][0]+eneinlog*(
                ParticleEnergyScaleFactors[irbin][1]+eneinlog*(
                ParticleEnergyScaleFactors[irbin][2]+eneinlog*(
                ParticleEnergyScaleFactors[irbin][3]+eneinlog*(
                ParticleEnergyScaleFactors[irbin][4]))));
  return (yin/ydis)*(energyin/energydis);
}

// we have commented out the use of RMS sinc it is calculated wrongly in the parametrization section
// (see KM3SD and KM3EventAction)
// so we use strictly poisson distribution until it is fixed
void KM3HAEnergyFlux::FindBins(G4int idbeam, G4double energyin, G4double distancein, G4double anglein)
{
  //we have 2 catogories of hadronic particles for the parametrization. Charged ones and neutral ones
  //since they show very different angular profiles
  if(idbeam == 211 || idbeam == -211 || idbeam == 2212 || idbeam == -2212 || idbeam == 321 || idbeam == -321)ibin=0;
  else if(idbeam == 130 || idbeam == 2112 || idbeam == -2112)ibin=1;
...
//tempoR  FluxRMS=(*keepEnergies)[ibin1]->GiveFluxRMS() + ratio*((*keepEnergies)[ibin2]->GiveFluxRMS() - (*keepEnergies)[ibin1]->GiveFluxRMS() );
...
//tempoR  G4double deviation=FluxRMS-Flux;// It is variance-mean.this should be >0 for polya, =0 for poisson.
//tempoR  if(deviation < 0.001){ //this case is for poisson (the limiting case of polya as n->oo is the poisson).
