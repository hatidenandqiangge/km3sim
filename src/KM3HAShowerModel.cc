...
void KM3HAShowerModel::InitializeFlux(char * infileParam,G4double Quantum_Efficiency,G4double TotCathodArea)
{
#ifndef G4DISABLE_PARAMETRIZATION
  myFlux =new KM3HAEnergyFlux(infileParam,Quantum_Efficiency,TotCathodArea,EnergyMin,EnergyMax);
#else
  ;
#endif
}
...
G4bool KM3HAShowerModel::IsApplicable(const G4ParticleDefinition& particleType)
{
  if(
...
G4bool KM3HAShowerModel::ModelTrigger(const G4FastTrack& fastTrack)
{
  ...
  //  if(fastTrack.GetPrimaryTrack()->GetDefinition()->GetBaryonNumber() != 0)  //if it is a baryon use kinetic energy 
  //    Energy = fastTrack.GetPrimaryTrack()->GetKineticEnergy();
  //  else Energy = fastTrack.GetPrimaryTrack()->GetTotalEnergy(); //if it is a meson use total energy
  Energy = (fastTrack.GetPrimaryTrack()->GetMomentum()).mag(); //in fact the parametrization creation is according to momentum
  return ( (Energy>=EnergyMin) && (Energy<=EnergyMax) && (materialName=="Water"));
}
		     ...
		     G4FastStep& fastStep)
{
  ...
  //  else primaryShowerEnergy = fastTrack.GetPrimaryTrack()->GetTotalEnergy(); //if it is a meson use total energy
  primaryShowerEnergy = (fastTrack.GetPrimaryTrack()->GetMomentum()).mag(); //in fact the parametrization creation is according to momentum
  ...
  //to the maximum of the photon emission (see also generation action in the EM and HA parametrization section)
  ...
  /*
  if(theProcess != NULL){
    G4cout << "KM3HAShowerModel::DoIt particle " << fastTrack.GetPrimaryTrack()->GetDefinition()->GetParticleName() 
  ...
  for(int it=0;it<TotalNumberOfTowers;it++){
	...
	if(distancein<MaxAbsDist2){
	  ...
	  for(G4int isa=0 ; isa<NumberOfSamples ; isa++){
	    ...
	    //short	    aMySD->InsertExternalHit(ic,theFastTime+aPE.time,originalInfo,angleDirection,-900);
