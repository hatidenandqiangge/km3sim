...
{
  static KM3PrimaryGeneratorAction* aGeneAction = (KM3PrimaryGeneratorAction*)(G4RunManager::GetRunManager()->GetUserPrimaryGeneratorAction());
  ...
  if( (aTrack->GetTrackStatus() == fStopAndKill) || (aTrack->GetTrackStatus() == fKillTrackAndSecondaries) )return fKill;
...
//   else return fKill;
  ...
  //   if(info != NULL){
  //     G4cout << "Muon "<<aTrack->GetTrackID()<<" with kinetic energy " <<aTrack->GetKineticEnergy() << " was created by particle ID "<<info->GetOriginalParentID()<<G4endl;
  //   }
  //   else{
  ...
  //We also kill any secondary high energy muon with range more that 4.4915581 meters (kinetic energy>1GeV), while we write its track information
  if(aTrack->GetParentID()==0){
    if(aTrack->GetDefinition()==G4MuonPlus::MuonPlusDefinition() ||
       ...
       aTrack->GetDefinition()==G4TauMinus::TauMinusDefinition() )return fKill;
  }
  else{
    if(aTrack->GetDefinition()==G4MuonPlus::MuonPlusDefinition() ||
       aTrack->GetDefinition()==G4MuonMinus::MuonMinusDefinition() ){
	...
	G4cout <<"Energetic Muons "<<theTime<<" "<<thePosition[0]<<" "<<thePosition[1]<<" "<<thePosition[2]<<" "<<theMomentum[0]<<" "<<theMomentum[1]<<" "<<theMomentum[2]<<G4endl;
      }
      return fKill;
    }
...
#ifndef G4DISABLE_PARAMETRIZATION
  //here we kill any generated muon with kinetic energy >1GeV that is not primary particle since we have added this to the genarator action
  if(aTrack->GetDefinition()==G4MuonPlus::MuonPlusDefinition() ||
     aTrack->GetDefinition()==G4MuonMinus::MuonMinusDefinition() ){
    if( (aTrack->GetParentID()!=0) && (aTrack->GetKineticEnergy() > 1.0*GeV) )return fKill;
  }
#endif
#endif
#endif
  ...
  if(aTrack->GetDefinition()!=G4OpticalPhoton::OpticalPhotonDefinition()){  //first check that is not a photon to save time
	...
	(aTrack->GetDefinition()->GetPDGMass()==0.0) )return fKill;
    ...
    if(aTrack->GetDefinition()!=G4MuonPlus::MuonPlusDefinition() && aTrack->GetDefinition()!=G4MuonMinus::MuonMinusDefinition()){
      if( (x0[2]<MyStDetector->bottomPosition) ||  (distanceRho2>detectorMaxRho2) || (x0[2]>MyStDetector->detectorMaxz) )return fKill;
#if !defined(G4MYEM_PARAMETERIZATION) && !defined(G4MYHA_PARAMETERIZATION)
#ifdef G4ENABLE_MIE
#ifndef G4DISABLE_PARAMETRIZATION
      //delta rays parametrization section. Works only for muons primaries (not for showers ve)
	...
	if( (aTrack->GetCreatorProcess()->GetProcessName()=="muIoni") && parentID<=aGeneAction->numberofParticles &&
	  ...
	  //// here we create the photons from direct emission and delta rays before the end of the event
	  //// in order for the memory usage not to grow too large (specially dealing with multi muon signal - showers)
	  //// We also create the these photons at the end of the event through NewStage (double code)
	  if(idprikeep->size() == 500000)CreateAllWaitingPhotons();
	  ////
	  //before	if(indexkeep == 0){
	  //before	  return fWaiting; //i do this in order to add the hits of the delta rays at the end of the event
	  //before	  indexkeep=1;
	  //before	}
      ...
      if( (distanceRho2>detectorMaxRho2) && (direction>0) )return fKill; //goes away while outside the can

      //first check if it is inside the can
      G4double rxy2=x0[0]*x0[0]+x0[1]*x0[1];
      if( (rxy2 < detectorMaxRho2) && (x0[2]>MyStDetector->bottomPosition) && (x0[2] < MyStDetector->detectorMaxz) )return fUrgent;
...
void KM3StackingAction::NewStage()
{
  ...
  if (thespeedmaxQE<0){
    ...
    if(myFlux==NULL)myFlux =new KM3EMDeltaFlux(MyStDetector->EMParametrization_FILE,MyStDetector->Quantum_Efficiency,MyStDetector->TotCathodArea);
  }
  ...
  static G4double distbin2=(100.0*cm)*(100.0*cm); //is the distance binning in the delta rays for gathering
  size_t arraysize=idprikeep->size();
  //  G4cout << "----------- size from stacking NewStage ----------- "<<arraysize<<G4endl;
  size_t counter=0;
  while (counter<arraysize){
	...
	counter++;
      ...
      if(step > 0.0){ //discard delta rays that are alone in distance inetrval (too low signal)
	p0 /= step;
	thispos /= depene;
	thistime /= depene;
	for(int it=0;it<TotalNumberOfTowers;it++){
	  ...
	  if(distancetower2<MaxAbsDist2){
		...
		for(G4int isa=0 ; isa<NumberOfSamples ; isa++){
		  ...
		  //short		  aMySD->InsertExternalHit(ic,theFastTime+aPE.time,originalInfo,angleDirection,-997);
...
void KM3StackingAction::CreateAllWaitingPhotons()
{
  ...
  static G4double distbin2=(100.0*cm)*(100.0*cm); //is the distance binning in the delta rays for gathering
  size_t arraysize=idprikeep->size();
  //  G4cout << "----------- size from stacking CreateAllWaitingPhotons ----------- "<<arraysize<<G4endl;
  size_t counter=0;
  while (counter<arraysize){
    G4int idpri=(*idprikeep)[counter];
    while( (counter<arraysize) && (idpri == (*idprikeep)[counter]) ){
	...
	counter++;
      ...
      if( p0this.mag2() > 0.0 )p0 = p0this;
      G4double step=p0.mag();
      if(step > 0.0){  //discard delta rays that are alone in distance inerval (too low signal)
	p0 /= step;
	thispos /= depene;
	thistime /= depene;
	for(int it=0;it<TotalNumberOfTowers;it++){
