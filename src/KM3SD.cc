...
G4bool KM3SD::ProcessHits(G4Step* aStep,G4TouchableHistory* ROhist)
{
  ...
  //newmie don't count photons that are primary particles and have not been scattered
  ...
  if(MyCollection->entries() <10000000){
    ...
    //newmie
    //here we disgard photons that have been scattered and are created with parametrization also in KM3Cherenkov
    //this for parametrization running. Not anymore, since these are killed in the G4OpMie scattering process
#ifdef G4TRACK_INFORMATION
    ...
    //	aStep->GetTrack()->SetTrackStatus(fStopAndKill); //it is killed, because it is useless
    //	return false;
    //      }
    //    }
    //#else
    //    G4Exception("KM3SD::ProcessHits: Parametrization application needs track information","",FatalException,"");
    //#endif
    //#endif
    //newmie
...
#ifdef G4TRACK_INFORMATION
    if(info==NULL)info= (KM3TrackInformation*)(aStep->GetTrack()->GetUserInformation());
