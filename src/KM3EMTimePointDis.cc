...
KM3EMTimePointDis::KM3EMTimePointDis(std::ifstream& infile, bool& ok)
{
  ...
  //  G4cout<<"KM3EMTimePointDis "<<angle<<" "<<Flux<<" "<<(*keepTh2Th3Num)[833]<<G4endl;
  ...
  if(ibinNum_Tot!=OMSolidAngleBins)G4Exception("Error calculated direction bins are not the same as in KM3SD\n","",FatalException,""); //number needs to change
  //////////////////////////////////////////////////  
}
...
onePE KM3EMTimePointDis::GetSamplePoint()
{
  ...
  G4double costh;
  G4double theta,phi;
  if(!IsThisValid)G4Exception("Error sampling point and time for null distribution\n","",FatalException,"");
  //first we sample a direction point using the cumulative keepTh2Th3Num [0-833]
  ...
  if(fabs(param)>1.0e-6){
    ...
    if(dm<700.0)theta=minval+(1.0/param)*log(1.0+G4UniformRand()*(exp(dm)-1.0));
    else{
      double rrr=G4UniformRand();
      if(rrr>0.0)theta=maxval+log(rrr)/param;
      else theta=minval;
    }
  }
  else theta=minval+G4UniformRand()*(maxval-minval);
  //next sample a phi
  ...
  if(fabs(param)>1.0e-6){
    double dm=param*(maxval-minval);
    if(dm<700.0)phi=minval+(1.0/param)*log(1.0+G4UniformRand()*(exp(dm)-1.0));
    else{
      double rrr=G4UniformRand();
      if(rrr>0.0)phi=maxval+log(rrr)/param;
      else phi=minval;
    }
  }
  else phi=minval+G4UniformRand()*(maxval-minval);
  //  G4cout<<"ibinNum23= "<<ibinNum23<<" theta= "<<theta<<" phi= "<<phi<<G4endl;
  //next we must find the time bins that belongs this photon
  //follows the limits of the bins
  static double 
    maxth2[17]={4.43924, //the definitions of the array and the next one is in KM3SD.cc
		6.27957,
		8.10961,
		9.93636,
		11.4783,
  ...
  static double 
    maxth3[52]={55.0,105.0,180.0,
		...
		45.0,110.0,180.0,
		40.0,75.0,180.0,
		30.0,45.0,95.0,180.0,
		40.0,75.0,110.0,180.0,
		30.0,45.0,85.0,145.0,180.0,
		40.0,75.0,115.0,180.0,
		40.0,95.0,180.0,
		45.0,85.0,180.0,
		40.0,110.0,180.0,
		65.0,120.0,180.0,
		180.0,
		180.0}; //dimension 52 should be equal to TimeSolidAngleBins
  static int
    indexes[18]={0,3,6,9,12,15,18,21,25,29,34,38,41,44,47,50,51,52};
  G4int ith;
  for(ith=0 ; ith<17 ; ith++)if(theta<maxth2[ith])break;
  if(ith>16)ith=16;
  G4int iph;
  for(iph=indexes[ith] ; iph<indexes[ith+1] ; iph++)if(phi<maxth3[iph])break;
  ...
  rrr=G4UniformRand();
  G4int ibint;
  for(ibint=TimeBins*cang23bin ; ibint<TimeBins*cang23bin+TimeBins ; ibint++){
    if(rrr < (*keepDis)[ibint])break;
  }
  ibint-=cang23bin*TimeBins;
  if(ibint<40)time=double(ibint)*0.5-10.0+G4UniformRand()*0.5;
  else if(ibint<60)time=double(ibint-40)+10.0+G4UniformRand();
  else if(ibint<80)time=double(ibint-60)*3.0+30.0+G4UniformRand()*3.0;
  else if(ibint<90)time=double(ibint-80)*11.0+90.0+G4UniformRand()*11.0;
  else if(ibint<106)time=double(ibint-90)*50.0+200.0+G4UniformRand()*50.0;
  else time=double(ibint-106)*200.0+1000.0+G4UniformRand()*200.0;
