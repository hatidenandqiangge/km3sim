//***** Generated by Geant4 Geometry Editor at  Thu Apr 04 19:40:34 GMT+03:00 2002 *****
...
#if !defined(G4MYEM_PARAMETERIZATION) && !defined(G4MYHA_PARAMETERIZATION) && !defined(G4DISABLE_PARAMETRIZATION)
...
void KM3Detector::FindDetectorRadius()
{
  ...
  G4cout << "Detector radius (m) and bottom position (m) " << detectorRadius/m <<" "<<bottomPosition/m<<G4endl;
  MyGenerator->PutFromDetector(detectorCenter,detectorMaxRho,detectorMaxz,bottomPosition);
}
...
void KM3Detector::SetUpVariables()
{
  ...
  if((infile=fopen(Parameter_File,"r"))==NULL){
    ...
    fCalc.setSystemOfUnits(1.e+3, 1./1.60217733e-25, 1.e+9, 1./1.60217733e-10,1.0, 1.0, 1.0);//asign default variables to evaluator (Geant4 Units)
    while( fscanf(infile,"%s",varname) != EOF){
      G4String thename=G4String(varname);
      if(thename==String0){
	readvalues[0]=1;
	fscanf(infile,"%s\n",expression);
	detectorDepth=fCalc.evaluate(expression);}          //this should be the detector depth at the center of the detector. It is used to calculate
                                                            //the seawater density using the compressibility of the sea water (see below)
      else if(thename==String1){
	readvalues[1]=1;
	fscanf(infile,"%s\n",expression);
	bottomPosition=fCalc.evaluate(expression);}          //this is the position of the bottom of the sea relative to detector center
      else if(thename==String2){
	readvalues[2]=1;
	fscanf(infile,"%s",expression);
	NUMENTRIES=(G4int)fCalc.evaluate(expression);
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; PPCKOV[i]=h_Planck*c_light/fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; PPCKOV[NUMENTRIES-1]=h_Planck*c_light/fCalc.evaluate(expression);}
      else if(thename==String3){
	readvalues[3]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; RINDEX_WATER[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; RINDEX_WATER[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String4){
	readvalues[4]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	fscanf(infile,"%s",expression) ; Water_Transparency=fCalc.evaluate(expression);
	fscanf(infile,"%s",expression) ; MaxRelDist=fCalc.evaluate(expression);              //How many absorption lengths optical photons can cross
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; ABSORPTION_WATER[i]=Water_Transparency/fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; ABSORPTION_WATER[NUMENTRIES-1]=Water_Transparency/fCalc.evaluate(expression);
#if (defined(G4MYEM_PARAMETERIZATION) || defined(G4MYHA_PARAMETERIZATION)) && !defined(G4MYK40_PARAMETERIZATION)
      //here we put a virtual absorption length (300m) to a large value to have more photons detected on large distances
      //next in KM3SD we calculate the probability of the photons according to the true absorption length we keep 
	...
	readvalues[5]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; RINDEX_GLASS[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; RINDEX_GLASS[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String6){
	readvalues[6]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; ABSORPTION_GLASS[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; ABSORPTION_GLASS[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String7){
	readvalues[7]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; RINDEX_GELL[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; RINDEX_GELL[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String8){
	readvalues[8]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; ABSORPTION_GELL[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; ABSORPTION_GELL[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String9){
	readvalues[9]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; RINDEX_AIR[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; RINDEX_AIR[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String10){
	readvalues[10]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; ABSORPTION_AIR[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; ABSORPTION_AIR[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String11){
	readvalues[11]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; RINDEX_CATH[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; RINDEX_CATH[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String12){
	readvalues[12]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; ABSORPTION_CATH[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; ABSORPTION_CATH[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String13){
	readvalues[13]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property (PPCKOV keyword)","",FatalException,"");
	fscanf(infile,"%s",expression) ; Quantum_Efficiency=fCalc.evaluate(expression);  //maximum quantum efficienscy, used in KM3Cherenkov
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; Q_EFF[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; Q_EFF[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String14){
	readvalues[14]=1;
	if(NUMENTRIES < 0)G4Exception("Wavelengths of Optical Photons must be set before setting any other Optical Property (PPCKOV keyword)","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES-1 ; i++){fscanf(infile,"%s",expression) ; SCATTER_WATER[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; SCATTER_WATER[NUMENTRIES-1]=fCalc.evaluate(expression);}
      else if(thename==String15){
	readvalues[15]=1;
	fscanf(infile,"%s\n",expression) ; MieModel=fCalc.evaluate(expression);}  //Mie model as of mie phase factors file
      else if(thename==String16){
	readvalues[16]=1;
	fscanf(infile,"%s",expression);
	NUMENTRIES_ANGLEACC=(G4int)fCalc.evaluate(expression);
	for(G4int i=0 ; i<NUMENTRIES_ANGLEACC-1 ; i++){fscanf(infile,"%s",expression) ; COSANGLES[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; COSANGLES[NUMENTRIES_ANGLEACC-1]=fCalc.evaluate(expression);}
      else if(thename=String17){
	readvalues[17]=1;
	if(NUMENTRIES_ANGLEACC < 0)G4Exception("Cosine angles values must be set before setting angular acceptance values","",FatalException,"");
	for(G4int i=0 ; i<NUMENTRIES_ANGLEACC-1 ; i++){fscanf(infile,"%s",expression) ; ACCEPTANCE[i]=fCalc.evaluate(expression);}
	fscanf(infile,"%s\n",expression) ; ACCEPTANCE[NUMENTRIES_ANGLEACC-1]=fCalc.evaluate(expression);}
      else G4Exception("Not a keyword I can recognize\n","",FatalException,"");
    }
    fclose(infile);
	...
	switch(i)
	  {
	  case 0 : G4Exception("detectorDepth not set","",FatalException,""); break ;
	  case 1 : G4Exception("bottomPosition not set","",FatalException,""); break ;
	  case 2 : G4Exception("PPCKOV not set","",FatalException,""); break ;
	  case 3 : G4Exception("RINDEX_WATER not set","",FatalException,""); break ;
	  case 4 : G4Exception("ABSORPTION_WATER not set","",FatalException,""); break ;
	  case 5 : G4Exception("RINDEX_GLASS not set","",FatalException,""); break ;
	  case 6 : G4Exception("ABSORPTION_GLASS not set","",FatalException,""); break ;
	  case 7 : G4Exception("RINDEX_GELL not set","",FatalException,""); break ;
	  case 8 : G4Exception("ABSORPTION_GELL not set","",FatalException,""); break ;
	  case 9 : G4Exception("RINDEX_AIR not set","",FatalException,""); break ;
	  case 10 : G4Exception("ABSORPTION_AIR not set","",FatalException,""); break ;
	  case 11 : G4Exception("RINDEX_CATH not set","",FatalException,""); break ;
	  case 12 : G4Exception("ABSORPTION_CATH not set","",FatalException,""); break ;
	  case 13 : G4Exception("Q_EFF not set","",FatalException,""); break ;
	  case 14 : G4Exception("MIE_WATER not set","",FatalException,""); break ;
	  case 15 : G4Exception("MIE_MODEL not set","",FatalException,""); break ;
	  case 16 : G4Exception("COS_ANGLES not set","",FatalException,""); break ;
	  case 17 : G4Exception("ANG_ACCEPT not set","",FatalException,""); break ;
	  }
      }
    }
  }
...
void KM3Detector::ConstructMaterials()
{
  ...
  //Earths Crust
  G4Material* Crust = new G4Material("Crust",2.6*g/cm3,8,kStateSolid,287.15*kelvin,1.0*atmosphere );
  ...
  //WATER---here is added the depedence of the salinity and density on the depth (UNESCO)--
  //At the surface the seawater density is approximately 1.029gr/cm3 (E.G.Anassontzis, "The NESTOR Site",
  //Proceedings of the 3rd NESTOR International Workshop, October 19-21, 1993, Pylos Greece, pp614-630)
  //based on the composition of ocean seawater for 35o/oo  www.soest.hawaii.edu/oceanography we have
  ...
  abundanceSO4  *= scale;
  abundanceHCO3 *= scale;
  //we convert to gr/kgr of the solution (seawater) for each element or composite 
  ...
  G4double abundanceH2O   = 1.0-(abundanceNa+abundanceMg+abundanceCa+abundanceK+abundanceCl+abundanceSO4+abundanceHCO3);
  //------------------the composites of sea water------------------------------------
  //if the material is not used to fill a specific logical volume the density is not used
  ...
  //calculate the density of sea water using the compressibility (Apostolis Thesis appendix C)
  ...
  G4double seawaterDensity=surfaceDensity*exp(gravity*surfaceDensity*Compressibility*detectorDepth);
  //-----------------------------------------------------------------
  G4Material* Water = new G4Material( "Water", seawaterDensity, 8, kStateLiquid, 287.15*kelvin, 1.0*atmosphere);
  ...
  //GLASS and GELL change (27/5/2005) of the composition of borosilicate glass according to
  ...
  G4Material* Glass = new G4Material("Glass",2.23*g/cm3,4,kStateSolid,287.15*kelvin,1.0*atmosphere );
  ...
  //density is 0.97g/cm3 (http://www.wackersilicones.com/documents/techdatasheets/silgel612.pdf)
  //Polydimethylsiloxane polymeres -- (C2H6OSi)n
  G4Material* Gell = new G4Material("Gell", 0.97*g/cm3, 4, kStateSolid, 287.15*kelvin, 1.0*atmosphere );
  ...
  // CATHOD -------------------------------------------------------------------------------
  G4Material* Cathod = new G4Material("Cathod", 22, 47.867*g/mole, 4.507*g/cm3,kStateSolid, 287.15*kelvin, 1.0*atmosphere );
  ...
  Properties_Water->AddProperty("ABSLENGTH",PPCKOV,ABSORPTION_WATER,NUMENTRIES);
#if (defined(G4MYEM_PARAMETERIZATION) || defined(G4MYHA_PARAMETERIZATION)) && !defined(G4MYK40_PARAMETERIZATION)
  Properties_Water->AddProperty("ABSLENGTH_TRUE",PPCKOV,ABSORPTION_WATER_TRUE,NUMENTRIES);
#endif
  ...
  Properties_Glass->AddProperty("ABSLENGTH", PPCKOV, ABSORPTION_GLASS, NUMENTRIES);
  ...
  Properties_Gell->AddProperty("ABSLENGTH", PPCKOV, ABSORPTION_GELL, NUMENTRIES);
  Properties_Gell->AddProperty("RINDEX",PPCKOV,RINDEX_GELL,NUMENTRIES);
  Gell->SetMaterialPropertiesTable(Properties_Gell);

  //AIR       -----------------------------------------------------------
  //absorption length of air set to 1 meter to prevent trapping of a photon inside air material
  ...
  Properties_Cath->AddProperty("ABSLENGTH", PPCKOV, ABSORPTION_CATH, NUMENTRIES);
  ...
  Properties_Cath->AddProperty("ANGULAR_ACCEPTANCE",COSANGLES,ACCEPTANCE,NUMENTRIES_ANGLEACC);
...
G4int KM3Detector::TotalPMTEntities(const G4VPhysicalVolume* aPVolume) const
{
  ...
  static std::vector<G4int> * aBenthosIDs;  //static in order to load the benthos (OMs)
#if !defined(G4MYEM_PARAMETERIZATION) && !defined(G4MYHA_PARAMETERIZATION) && !defined(G4DISABLE_PARAMETRIZATION)
  static std::vector<G4int> * aTowerBenthosIDs;  //static in order to load the benthos (OMs) in towers //new towers
#endif
  static std::vector<G4int> * aCathodsIDs;  //static in order to load the cathods (Cathods)

  //  G4cout <<Depth<<" "<<aPVolume->GetCopyNo()<<" "<<aPVolume->GetName()<<G4endl; //tempotest
  Hist[Depth]=aPVolume->GetCopyNo();
  Depth++;
  RotationMatr = RotationMatr*aPVolume->GetObjectRotationValue();
  //  if(aPVolume->GetName() == "CathodVolume_PV"){ //for newgeant  add "_PV" at the end of physical volume name
  if( (aPVolume->GetName()).contains("CathodVolume") ){ //for newgeant  add "_PV" at the end of physical volume name
    ...
    G4double CathodHeight=-1.0*mm; //set default to begative, since it is not applicable to spherical cathods
    if(aPVolume->GetLogicalVolume()->GetSolid()->GetEntityType()==G4String("G4Sphere")){
      CathodRadius = ((G4Sphere*) aPVolume->GetLogicalVolume()->GetSolid())->GetOuterRadius();
      G4double InnerRadius=((G4Sphere*) aPVolume->GetLogicalVolume()->GetSolid())->GetInnerRadius();
      if( (CathodRadius-InnerRadius)<1.001*mm)CathodRadius=0.5*(CathodRadius+InnerRadius); //applicable mainly to shell type cathods (EM)
    }      
    else if(aPVolume->GetLogicalVolume()->GetSolid()->GetEntityType()==G4String("G4Tubs")){
      CathodRadius = ((G4Tubs*) aPVolume->GetLogicalVolume()->GetSolid())->GetOuterRadius(); //applicable to thin tube cathods (normal run)
      CathodHeight = ((G4Tubs*) aPVolume->GetLogicalVolume()->GetSolid())->GetZHalfLength();
      CathodHeight *= 2.0; //full height
    }
    ///////////////////////////////////////////
    allCathods->addCathod(trans,Position,Direction,CathodRadius,CathodHeight,Depth-1);
    for(G4int i=1 ; i<Depth ; i++)allCathods->addToTree(Hist[i]);
    //G4cout << Depth <<" "<<Hist[0]<<" "<<Hist[1]<<" "<<Hist[2]<<" "<<Hist[3]<<" "<<Hist[4]<<" "<<Hist[5]<<G4endl; //tempotest
    //G4cout << Position <<" "<<Direction<<G4endl; //tempotest
    aCathodsIDs->push_back(Cathods);
    Cathods++;
  }
  else{
    if( (aPVolume->GetName()).contains("OMVolume") ){ //for newgeant  add "_PV" at the end of physical volume name
      ...
      if(aPVolume->GetLogicalVolume()->GetSolid()->GetEntityType()==G4String("G4Sphere")){
	aOM->radius = ((G4Sphere*) aPVolume->GetLogicalVolume()->GetSolid())->GetOuterRadius();
      }
      else if(aPVolume->GetLogicalVolume()->GetSolid()->GetEntityType()==G4String("G4Tubs")){
	G4double zLength=((G4Tubs*) aPVolume->GetLogicalVolume()->GetSolid())->GetZHalfLength();
	G4double oRadius=((G4Tubs*) aPVolume->GetLogicalVolume()->GetSolid())->GetOuterRadius();
	...
	aPVolume->GetLogicalVolume()->GetSolid()->CalculateExtent(kXAxis,voxelLimits,affineTransform,xmin,xmax);
	aPVolume->GetLogicalVolume()->GetSolid()->CalculateExtent(kYAxis,voxelLimits,affineTransform,ymin,ymax);
	aPVolume->GetLogicalVolume()->GetSolid()->CalculateExtent(kZAxis,voxelLimits,affineTransform,zmin,zmax);
    ...
    if( (aPVolume->GetName()).contains("StoreyVolume") ){ //for newgeant  add "_PV" at the end of physical volume name
      StoreysPositions * aStorey = (StoreysPositions*)malloc(sizeof(StoreysPositions));
      aStorey->position = AffineTrans.TransformPoint(aPVolume->GetObjectTranslation());
...
#if !defined(G4MYEM_PARAMETERIZATION) && !defined(G4MYHA_PARAMETERIZATION) && !defined(G4DISABLE_PARAMETRIZATION)
    if( (aPVolume->GetName()).contains("TowerVolume") ){ //new towers //for newgeant  add "_PV" at the end of physical volume name
      TowersPositions * aTower = (TowersPositions*)malloc(sizeof(TowersPositions));
      aTower->position = AffineTrans.TransformPoint(aPVolume->GetObjectTranslation());
...
#endif
    G4AffineTransform tempoaffine(aPVolume->GetObjectRotationValue().inverse(),aPVolume->GetObjectTranslation());
    AffineTrans = tempoaffine*AffineTrans;
    for(G4int i=0 ; i<aPVolume->GetLogicalVolume()->GetNoDaughters() ; i++){
      //the following is to fix new GDML that does not apply a copyNumber to physical volumes
...
G4VPhysicalVolume* KM3Detector::Construct( )
{
  ...
  //newgeant fWorld =  (G4VPhysicalVolume *)GDMLProcessor::GetInstance()->GetWorldVolume();
  G4GDMLParser parser; //newgeant
  parser.Read(Geometry_File); //newgeant
  fWorld =  (G4VPhysicalVolume *)parser.GetWorldVolume(); //newgeant
  if( fWorld == 0 )G4Exception("World volume not set properly check your setup selection criteria or GDML input!","",FatalException,"");
  ...
  //next find the Cathod && Dead logical volumes and assign them the sensitive detectors
  ...
  if(DrawDetector){
    ...
    G4cout << "Available colors are: white gray(grey) black red green blue cyan magenta yellow none" <<G4endl;
  }
  for(size_t i=0 ; i<theSize ; i++){
    ...
    if(DrawDetector){
      ...
      if( colour == G4String("none") )aLogicalVolume->SetVisAttributes(G4VisAttributes::Invisible);
      else{
	G4VisAttributes * MyVis;
	G4bool ok=true;
	if( colour == G4String("white"))MyVis = new G4VisAttributes(G4Colour::White());
	else if( colour == G4String("gray") || colour == G4String("grey"))MyVis = new G4VisAttributes(G4Colour::Gray());
	else if( colour == G4String("black"))MyVis = new G4VisAttributes(G4Colour::Black());
	else if( colour == G4String("red"))MyVis = new G4VisAttributes(G4Colour::Red());
	else if( colour == G4String("green"))MyVis = new G4VisAttributes(G4Colour::Green());
	else if( colour == G4String("blue"))MyVis = new G4VisAttributes(G4Colour::Blue());
	else if( colour == G4String("cyan"))MyVis = new G4VisAttributes(G4Colour::Cyan());
	else if( colour == G4String("magenta"))MyVis = new G4VisAttributes(G4Colour::Magenta());
	else if( colour == G4String("yellow"))MyVis = new G4VisAttributes(G4Colour::Yellow());
	else ok=false;
	if(ok){
	...
	else aLogicalVolume->SetVisAttributes(G4VisAttributes::Invisible);
      }
    }
    
    ////////////////////////////////////////////////////
    //    if( (aLogicalVolume->GetName() == aString1) || (aLogicalVolume->GetName() == aString2) ){
    if( ( (aLogicalVolume->GetName()).contains(aString1) ) || ( (aLogicalVolume->GetName()).contains(aString2) ) ){
  ...
  //--------Write the header of the outfile and the Cathods Position, Direction and History Tree
  G4int nnn0=0;
  G4int nnn1=1;
  G4int nben=allCathods->GetNumberOfCathods();

  //here in case of antares evt format I should write something general about simulation
  ...
  if(!useANTARESformat){
    fprintf(outfile,"%d %d %d %d %d %d %d %d %d %d %f %f %d %d\n",nnn0,nnn1,nnn1,nnn1,nben,nnn0,nnn0,nnn0,nnn0,nnn0,Water_Transparency,Quantum_Efficiency,nnn0,nnn0);
  ...
  TotCathodArea=CaPerOM*pi*allCathods->GetCathodRadius(0)*allCathods->GetCathodRadius(0); //this is valid only if at simulation level (not EM or HA param) all cathods have the same radius. Easy to change to account for a detector with varius cathod types
  ...
  myEMShowerModel->InitializeFlux(EMParametrization_FILE,Quantum_Efficiency,TotCathodArea);
  myEMShowerModel->aMySD=aMySD;

#ifdef G4HADRONIC_COMPILE
  myHAShowerModel = new KM3HAShowerModel("haShowerModel",defRegion);
  myHAShowerModel->myStDetector=this;
  myHAShowerModel->InitializeFlux(HAParametrization_FILE,Quantum_Efficiency,TotCathodArea);
...
{
  //allmyBenthos keep for each sphere the positions and radii of the PMts that are in the current spheres
...
void KM3Detector::splitSpheresCluster(std::vector<StoreysPositions*> * allmyStoreys, Spheres* mySphere)
{
  static G4int depth=0;
  ...
  howmanySpheres++;
  //  G4cout <<howmanySpheres <<" Number of benthos " << howmanyStoreys <<G4endl;
  ...
  //load all benthos indexing in array ALLSTOREYS[1000] (needed only for clustering)
  for (G4int isto=0 ; isto<howmanyStoreys ; isto++){
    ALLSTOREYS[isto]=-1;
  }
  ...
  for(G4int i=0 ; i<numofCenters ; i++){
    ...
    centers[i][0]=center[0]+radiusRANDOM*sin(thetaRANDOM)*cos(phiRANDOM);
    centers[i][1]=center[1]+radiusRANDOM*sin(thetaRANDOM)*sin(phiRANDOM);
  ...
  G4int iter=0;
  while(nmove>0){  //while the iteration process has not converged to an acceptable solution
    nmove=0;
    for(G4int i=0 ; i<howmanyStoreys ; i++){   //find each storey to what center belongs (is closer)
	...
	if(dist<distm){distm=dist; jmin=j;}
      }
      if( ALLSTOREYS[i] != jmin){ALLSTOREYS[i]=jmin ; nmove++;}
    }
    ...
    if(nmove==0){  //if it has converged see if the solution is acceptable
      for(G4int j=0 ; j<numofCenters ; j++){
	imanystoreysarr[j]=0;
	for(G4int i=0 ; i<howmanyStoreys ; i++)if( ALLSTOREYS[i]==j )imanystoreysarr[j]++;
      }    
      for(G4int j=0 ; j<numofCenters ; j++){
	if( (imanystoreysarr[j]<imanymin) ||
	    (imanystoreysarr[j]>imanymax) ) igood=0;
      }
      if(igood==0){  //if it is not acceptable then find randomly other centers
	iter++;
	imanyminMAX=int(fmax(double(imanyminMAX),fmin(imanystoreysarr[0],imanystoreysarr[1]))); //for only 2 centers
	if(iter>1000){
	...
	for(G4int j=0 ; j<numofCenters ; j++){
	  ...
	  thetaRANDOM=M_PI*drand48();
	  phiRANDOM=2*M_PI*drand48();
	  centers[j][0]=center[0]+radiusRANDOM*sin(thetaRANDOM)*cos(phiRANDOM);
	  centers[j][1]=center[1]+radiusRANDOM*sin(thetaRANDOM)*sin(phiRANDOM);
	...
	for(G4int j=0 ; j<numofCenters ; j++){
	  centers[j][0]=0.0;centers[j][1]=0.0;centers[j][2]=0.0;
    ...
    else nmove=10; //the solution is not good and do again
  }

  //------------end of clustering---------------------------
  //load in linked list the benthos of each center
  std::vector<StoreysPositions*> * positivemyStoreys = new std::vector<StoreysPositions*>;
  std::vector<StoreysPositions*> * negativemyStoreys = new std::vector<StoreysPositions*>;
  for(G4int i=0 ; i<howmanyStoreys ; i++){
