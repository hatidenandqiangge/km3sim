...
KM3EMEnergyFlux::KM3EMEnergyFlux(char* infileParam,G4double QEmax, G4double TotCathodArea, G4int NEner, G4double MBstRat)
{
  NEnergies=NEner;
  ...
  for(G4int i=0 ; i<NEnergies ; i++){
    ...
    if(EnergyMin > aDistanceFlux->GiveEnergy())EnergyMin=aDistanceFlux->GiveEnergy();
    if(EnergyMax < aDistanceFlux->GiveEnergy())EnergyMax=aDistanceFlux->GiveEnergy();
...
{
  if(log10(TheE)<EnergyMin || log10(TheE/MaxBoostRatio)>EnergyMax)return false; //linear extrapolation is used for up to MaxBoostRatio times the maximum energy
  else return true;
}
...
void KM3EMEnergyFlux::FindBins(G4double energyin,G4double distancein, G4double anglein)
{
  ...
  ratio=(TheE-(*keepEnergies)[ibin1]->GiveEnergy())/((*keepEnergies)[ibin2]->GiveEnergy() - (*keepEnergies)[ibin1]->GiveEnergy() );
...
{
  if(G4UniformRand()<ratio)return(*keepEnergies)[ibin1]->GetSamplePoint();
  else return (*keepEnergies)[ibin2]->GetSamplePoint();
}
