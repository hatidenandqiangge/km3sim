...
void KM3Physics::ConstructProcess()
{
  ...
  ConstructEM();
#ifdef G4HADRONIC_COMPILE 
  ConstructHA();  //construct hadronic processes only in case of Pythia input (for apparent reasons)
#endif
  ConstructGeneral();
  ConstructOP();
  ...
  //     G4cout << "------------------ "<<particleName<<" "<<isi<<" ------------------"<<G4endl;
  //   else
  //     G4cout << "----SHORTSHORT---- "<<particleName<<" "<<isi<<" ----SHORTSHORT----"<<G4endl;
...
// newgeant #include "G4MultipleScattering.hh" //in version 4.9.3.p02 geant4 threatens than G4MultipleScattering class will be removed in next releases, so the next ones are assigned for e+-, mu+-, hadrons and ions, respectively 
...
void KM3Physics::ConstructEM()
{
  ...
  while( (*theParticleIterator)() ){
    ...
    // gamma conversion to e+e- pair
      ...
      //      aGammaConversion->SetPhysicsTableBining(lowE,highE,nBins);     //not applicable to Geant 4.8
      ...
      highE=100.0*PeV; //100TeV before
      nBins=220;  //120 before
      //      aComptonScattering->SetPhysicsTableBining(lowE,highE,nBins);  //not applicable to Geant 4.8
    ...
    //electron
      ...
      //      G4cout<<"lower lambda e-"<<((G4eIonisation*)theeminusIonisation)->GetLowerBoundLambda()<<G4endl;
      //
      // add processes
    ...
    //positron
      ...
      //      G4VProcess* theAnnihiToMuPair          = new G4AnnihiToMuPair();  //is not likely to happen
      //      ((G4eIonisation*)theeplusIonisation)->SetLowerBoundLambda(0.00024);
      //      G4cout<<"lower lambda e+"<<((G4eIonisation*)theeplusIonisation)->GetLowerBoundLambda()<<G4endl;
      //
      // add processes
    ...
    //muon  
      ...
      //      G4cout<<"lower lambda pair m "<<((G4MuPairProduction*)aPairProduction)->GetLowerBoundLambda()<<G4endl;
      //
      // add processes
      ...
      if(particleName == "mu-")pmanager->AddRestProcess(new G4MuonMinusCaptureAtRest);
...
void KM3Physics::ConstructHA()
{
  ...
  G4HadronElasticPhysics* theHadronElasticPhysics = new G4HadronElasticPhysics(2); //verbose level
  theHadronElasticPhysics->ConstructProcess();

  //hadron physics (inelastic). from HadronPhysicsQGSP_FTFP_BERT builder
  HadronPhysicsQGSP_FTFP_BERT* theQGSP_FTFP_BERT = new HadronPhysicsQGSP_FTFP_BERT("hadron",true);
  theQGSP_FTFP_BERT->SetVerboseLevel(0);
  theQGSP_FTFP_BERT->ConstructProcess();

  //put elastic and inelastic models max energy to 100PeV only for barions and mesons.
  //since I dont want to dublicate the QGSP_FTFP_BERT code here and since at this moment
  //I don't have access to HadronicProcessStore, the only way to change the upper limit of the
  //high energy models of many barions and mesons is to access the Process through the particle manager
  //and access the highest energy interaction model of this process through the the G4EnergyRangeManager
  theParticleIterator->reset();
  while( (*theParticleIterator)() ){
    G4ParticleDefinition* particle = theParticleIterator->value();
    G4String particleName = particle->GetParticleName();
    if( particleName == "anti_lambda" || 
      ...
      for( G4int iii=0 ; iii<isi ; iii++){
      	G4VProcess* app=(*aVector)[iii];
	//	G4cout <<"PPPPPnames "<<particleName<<" "<< app->GetProcessName() <<" "<<app->GetProcessSubType()<<G4endl;
      	if(app->GetProcessSubType() == 121 || app->GetProcessSubType() == 111 ){ //is hadronic inelastic or elastic process
	  ...
	  G4cout << aHp->GetProcessName()<<G4endl;
  	  ...
  	  if(aMaterial == NULL || anElement == NULL){
  	    G4Exception("Material Water and one Element of it not found in ConstructHA","",FatalException,"");
  	  }else{
  	    G4HadronicInteraction* anIn = aMan->GetHadronicInteraction(99*TeV,aMaterial,anElement);
  ...
  //next add also inelastic processes for ions not included in HadronPhysicsQGSP_FTFP_BERT builder
  ...
  //Deuteron
  G4DeuteronInelasticProcess* theDeuteronInelastic = new G4DeuteronInelasticProcess();
...
void KM3Physics::ConstructHA()
{
  ...
  //  theHEModel->SetQuasiElasticChannel(new G4QuasiElasticChannel); //include both but only the first is used
  ...
  while( (*theParticleIterator)() ){
    ...
    if (particleName == "pi+") {
      ...
      G4LEPionPlusInelastic* theLEPionPlusInelasticModel = new G4LEPionPlusInelastic();
      theLEPionPlusInelasticModel->SetMinEnergy(1.4*GeV);
      G4HEPionPlusInelastic* theHEPionPlusInelasticModel = new G4HEPionPlusInelastic();
      ...
      G4LEPionMinusInelastic* theLEPionMinusInelasticModel = new G4LEPionMinusInelastic();
      theLEPionMinusInelasticModel->SetMinEnergy(1.4*GeV);
      G4HEPionMinusInelastic* theHEPionMinusInelasticModel = new G4HEPionMinusInelastic();
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEKaonPlusInelastic* theLEKaonPlusInelasticModel = new G4LEKaonPlusInelastic();
      G4HEKaonPlusInelastic* theHEKaonPlusInelasticModel = new G4HEKaonPlusInelastic();
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEKaonZeroSInelastic* theLEKaonZeroSInelasticModel = new G4LEKaonZeroSInelastic();
      G4HEKaonZeroInelastic* theHEKaonZeroInelasticModel = new G4HEKaonZeroInelastic();
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEKaonZeroLInelastic* theLEKaonZeroLInelasticModel = new G4LEKaonZeroLInelastic();
      G4HEKaonZeroInelastic* theHEKaonZeroInelasticModel = new G4HEKaonZeroInelastic();
      theHEKaonZeroInelasticModel->SetMaxEnergy(100*PeV);
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEKaonMinusInelastic* theLEKaonMinusInelasticModel = new G4LEKaonMinusInelastic();   
      G4HEKaonMinusInelastic* theHEKaonMinusInelasticModel = new G4HEKaonMinusInelastic();   
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEProtonInelastic* theLEProtonInelasticModel = new G4LEProtonInelastic();
      theLEProtonInelasticModel->SetMinEnergy(6.0*GeV);
      G4HEProtonInelastic* theHEProtonInelasticModel = new G4HEProtonInelastic();
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEAntiProtonInelastic* theLEAntiProtonInelasticModel = new G4LEAntiProtonInelastic();
      G4HEAntiProtonInelastic* theHEAntiProtonInelasticModel = new G4HEAntiProtonInelastic();
      ...
      //      theAntiProtonInelasticProcess->RegisterMe(theHEModel); //it was missing before
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LENeutronInelastic* theLENeutronInelasticModel = new G4LENeutronInelastic();
      theLENeutronInelasticModel->SetMinEnergy(6.0*GeV);
      G4HENeutronInelastic* theHENeutronInelasticModel = new G4HENeutronInelastic();
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEAntiNeutronInelastic* theLEAntiNeutronInelasticModel = new G4LEAntiNeutronInelastic();
      G4HEAntiNeutronInelastic* theHEAntiNeutronInelasticModel = new G4HEAntiNeutronInelastic();
      ...
      //      theAntiNeutronInelasticProcess->RegisterMe(theHEModel); //it was missing before
      ...
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LELambdaInelastic* theLELambdaInelasticModel = new G4LELambdaInelastic();
      G4HELambdaInelastic* theHELambdaInelasticModel = new G4HELambdaInelastic();
      ...
      //      theLambdaInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theLambdaInelasticProcess);

    } else if (particleName == "anti_lambda") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEAntiLambdaInelastic* theLEAntiLambdaInelasticModel = new G4LEAntiLambdaInelastic();
      G4HEAntiLambdaInelastic* theHEAntiLambdaInelasticModel = new G4HEAntiLambdaInelastic();
      theHEAntiLambdaInelasticModel->SetMaxEnergy(100*PeV);
      G4AntiLambdaInelasticProcess* theAntiLambdaInelasticProcess = new G4AntiLambdaInelasticProcess;
      ...
      //      theAntiLambdaInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theAntiLambdaInelasticProcess);

    } else if (particleName == "sigma-") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LESigmaMinusInelastic* theLESigmaMinusInelasticModel = new G4LESigmaMinusInelastic();
      G4HESigmaMinusInelastic* theHESigmaMinusInelasticModel = new G4HESigmaMinusInelastic();
      theHESigmaMinusInelasticModel->SetMaxEnergy(100*PeV);
      G4SigmaMinusInelasticProcess* theSigmaMinusInelasticProcess = new G4SigmaMinusInelasticProcess;
      ...
      //      theSigmaMinusInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theSigmaMinusInelasticProcess);

    } else if (particleName == "anti_sigma-") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEAntiSigmaMinusInelastic* theLEAntiSigmaMinusInelasticModel = new G4LEAntiSigmaMinusInelastic();
      G4HEAntiSigmaMinusInelastic* theHEAntiSigmaMinusInelasticModel = new G4HEAntiSigmaMinusInelastic();
      theHEAntiSigmaMinusInelasticModel->SetMaxEnergy(100*PeV);
      G4AntiSigmaMinusInelasticProcess* theAntiSigmaMinusInelasticProcess = new G4AntiSigmaMinusInelasticProcess;
      ...
      //      theAntiSigmaMinusInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theAntiSigmaMinusInelasticProcess);

    } else if (particleName == "sigma+") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LESigmaPlusInelastic* theLESigmaPlusInelasticModel = new G4LESigmaPlusInelastic();
      G4HESigmaPlusInelastic* theHESigmaPlusInelasticModel = new G4HESigmaPlusInelastic();      
      theHESigmaPlusInelasticModel->SetMaxEnergy(100*PeV);
      G4SigmaPlusInelasticProcess* theSigmaPlusInelasticProcess = new G4SigmaPlusInelasticProcess;
      ...
      //      theSigmaPlusInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theSigmaPlusInelasticProcess);

    } else if (particleName == "anti_sigma+") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEAntiSigmaPlusInelastic* theLEAntiSigmaPlusInelasticModel = new G4LEAntiSigmaPlusInelastic();
      G4HEAntiSigmaPlusInelastic* theHEAntiSigmaPlusInelasticModel = new G4HEAntiSigmaPlusInelastic();
      theHEAntiSigmaPlusInelasticModel->SetMaxEnergy(100*PeV);
      G4AntiSigmaPlusInelasticProcess* theAntiSigmaPlusInelasticProcess = new G4AntiSigmaPlusInelasticProcess;
      ...
      //      theAntiSigmaPlusInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theAntiSigmaPlusInelasticProcess);
 
    } else if (particleName == "xi0") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEXiZeroInelastic* theLEXiZeroInelasticModel = new G4LEXiZeroInelastic();
      G4HEXiZeroInelastic* theHEXiZeroInelasticModel = new G4HEXiZeroInelastic();
      ...
      //      theXiZeroInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theXiZeroInelasticProcess);

    } else if (particleName == "anti_xi0") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEAntiXiZeroInelastic* theLEAntiXiZeroInelasticModel = new G4LEAntiXiZeroInelastic();
      G4HEAntiXiZeroInelastic* theHEAntiXiZeroInelasticModel = new G4HEAntiXiZeroInelastic();
      theHEAntiXiZeroInelasticModel->SetMaxEnergy(100*PeV);
      G4AntiXiZeroInelasticProcess* theAntiXiZeroInelasticProcess = new G4AntiXiZeroInelasticProcess;
      ...
      //      theAntiXiZeroInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theAntiXiZeroInelasticProcess);

    } else if (particleName == "xi-") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEXiMinusInelastic* theLEXiMinusInelasticModel = new G4LEXiMinusInelastic();
      G4HEXiMinusInelastic* theHEXiMinusInelasticModel = new G4HEXiMinusInelastic();
      ...
      //      theXiMinusInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theXiMinusInelasticProcess);

    } else if (particleName == "anti_xi-") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEAntiXiMinusInelastic* theLEAntiXiMinusInelasticModel = new G4LEAntiXiMinusInelastic();
      G4HEAntiXiMinusInelastic* theHEAntiXiMinusInelasticModel = new G4HEAntiXiMinusInelastic();
      theHEAntiXiMinusInelasticModel->SetMaxEnergy(100*PeV);
      G4AntiXiMinusInelasticProcess* theAntiXiMinusInelasticProcess = new G4AntiXiMinusInelasticProcess;
      ...
      //      theAntiXiMinusInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theAntiXiMinusInelasticProcess);
    } else if (particleName == "omega-") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEOmegaMinusInelastic* theLEOmegaMinusInelasticModel = new G4LEOmegaMinusInelastic();
      G4HEOmegaMinusInelastic* theHEOmegaMinusInelasticModel = new G4HEOmegaMinusInelastic();
      theHEOmegaMinusInelasticModel->SetMaxEnergy(100*PeV);
      G4OmegaMinusInelasticProcess* theOmegaMinusInelasticProcess = new G4OmegaMinusInelasticProcess;
      ...
      //      theOmegaMinusInelasticProcess->RegisterMe(theHEModel); //it was missing before
      pmanager->AddDiscreteProcess(theOmegaMinusInelasticProcess);

    } else if (particleName == "anti_omega-") {
      pmanager->AddDiscreteProcess(theElasticProcess);
      G4LEAntiOmegaMinusInelastic* theLEAntiOmegaMinusInelasticModel = new G4LEAntiOmegaMinusInelastic(); 
      G4HEAntiOmegaMinusInelastic* theHEAntiOmegaMinusInelasticModel = new G4HEAntiOmegaMinusInelastic(); 
      theHEAntiOmegaMinusInelasticModel->SetMaxEnergy(100*PeV);
      G4AntiOmegaMinusInelasticProcess* theAntiOmegaMinusInelasticProcess = new G4AntiOmegaMinusInelasticProcess;
      ...
      //      theAntiOmegaMinusInelasticProcess->RegisterMe(theHEModel); //it was missing before
...
void KM3Physics::ConstructGeneral() {
  ...
  while( (*theParticleIterator)() ){
	...
	// set ordering for PostStepDoIt and AtRestDoIt. Muons do not decay but are captured (dense medium). Only for Hadronic runs
	if(particle->GetParticleName() == "mu-"){
	  ...
	  pmanager ->SetProcessOrdering(theDecayProcess, idxPostStep);
#ifdef G4HADRONIC_COMPILE 
	  if( (particle->GetParticleName() != "pi-") && (particle->GetParticleName() != "kaon-") )
#endif
	    pmanager ->SetProcessOrdering(theDecayProcess, idxAtRest);
	}
      }
...
void KM3Physics::AddParameterisation()
{
  ...
  //  while( (*theParticleIterator)() ){
  //    G4ParticleDefinition* particle = theParticleIterator->value();
  //    if(!particle->IsShortLived()){         //ShortLived Particles have not process manager (they are not transported but decay immediately)
  //      G4ProcessManager* pmanager = particle->GetProcessManager();
