...
void KM3Cathods::addCathod(const G4Transform3D& trans, const G4ThreeVector& Pos,const G4ThreeVector& Dir,const G4double Radius, const G4double Height, const G4int Dep)
{
...
void KM3Cathods::addToTree(const G4int hist)
{
  ...
  if((G4int)(theCathods[NumOfCathods-1]->Tree->size()) > theCathods[NumOfCathods-1]->Depth) G4cout<< "Warning: You add more History than declared" <<G4endl;
}
...
void KM3Cathods::PrintAllCathods(FILE* outfile)
{
  for(G4int i=0;i<NumOfCathods ; i++){
    ...
    fprintf(outfile,"%.6e %.6e %.6e %.6e %.6e %.6e\n",
	    theCathods[i]->Position(0)/cm,
	    theCathods[i]->Position(1)/cm,
	    theCathods[i]->Position(2)/cm,
	    theCathods[i]->Direction(0),
	    theCathods[i]->Direction(1),
	    theCathods[i]->Direction(2));
  }
}
