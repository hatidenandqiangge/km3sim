...
#include <stdio.h>
#include "G4OpMie.hh"
#if (defined(G4MYLASER_PARAMETERIZATION) && defined(G4TRACK_INFORMATION)) || (!defined(G4DISABLE_PARAMETRIZATION) && defined(G4TRACK_INFORMATION)) //newmie
...
//
G4VParticleChange* 
G4OpMie::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
...
#else
	G4Exception("G4OpMie::PostStepDoIt: Parametrization application needs track information","",FatalException,"");
#endif
#endif
	//newmie
	...
	G4ThreeVector NewPolarization = OldPolarization - OldPolarization.dot(NewMomentumDirection)*NewMomentumDirection;
	...
	aParticleChange.ProposeMomentumDirection(NewMomentumDirection);
        ...
        if (verboseLevel>0) {
	  printf("OldNew All %13.6le %13.6le %13.6le %13.6le %13.6le %13.6le %13.6le %13.6le %13.6le %13.6le %13.6le %13.6le\n",
		 OldMomentumDirection.x(),OldMomentumDirection.y(),OldMomentumDirection.z(),
		 NewMomentumDirection.x(),NewMomentumDirection.y(),NewMomentumDirection.z(),
...
// BuildThePhysicsTable for the Mie Scattering process (reads only the phase function model)
// --------------------------------------------------------
//
void G4OpMie::BuildThePhysicsTable()
{
  ...
  if(!thePhaseFactors){
    ...
    if((infilePF=fopen("MiePhaseFactors.in","r")) ==NULL){
      ...
      for(G4int i=0 ; i<56 ; i++){ //read the phase functions from measurements in shallow waters
	fscanf(infilePF,"%lf %lf %lf %lf %lf %lf %lf\n",&c0,&c1,&c2,&c3,&c4,&c5,&c6);
	PhaseFactors* aPhaseFactors=(PhaseFactors*)malloc(sizeof(PhaseFactors));
	aPhaseFactors->c0=1.0/c0;aPhaseFactors->c1=c1;aPhaseFactors->c2=c2;aPhaseFactors->c3=c3;
	aPhaseFactors->c4=c4;aPhaseFactors->c5=c5;aPhaseFactors->c6=c6;
	thePhaseFactors->push_back(aPhaseFactors);
      }
      //read the phase functions parametrized in deep water
      //first one is f4 model. c1 is parameter (a), c0 is the maximum of sin(th)*Ph(th)
      //Ph(th)=1/(1+a^2-2*a*cos(th))^(3/2) unnormalized
      fscanf(infilePF,"%lf %lf\n",&c0,&c1);
      PhaseFactors* aPhaseFactors=(PhaseFactors*)malloc(sizeof(PhaseFactors));
      aPhaseFactors->c0=1.0/c0;aPhaseFactors->c1=c1;
      thePhaseFactors->push_back(aPhaseFactors);
      
      //second one is p0.0075 model. c1 is parameter (p), the rayleigh scattering contribution
      //c2 is parameter (a) of rayleigh contribution, c3 parameter (b) of rayleigh g(th)=a*(1+b*cos^2(th))
  ...
  for (G4int i=0 ; i < numOfMaterials; i++){
	...
	DIndexPhaseFunction = aMaterialPropertiesTable->GetConstProperty("MIEPHASE");
...
G4double G4OpMie::PhaseFunction(G4double angle)
{
  ...
  if(IndexPhaseFunction < 56){
    ...
    val=c0*std::sin(angle)*exp(x*(c1+x*(c2+x*(c3+x*(c4+x*(c5+x*c6))))));
  }
  else if(IndexPhaseFunction==56){//f4 model
    c0=(*(thePhaseFactors))[IndexPhaseFunction]->c0;
    c1=(*(thePhaseFactors))[IndexPhaseFunction]->c1;
    val=c0*std::sin(angle)/std::pow(1+c1*c1-2*c1*std::cos(angle),1.5);
  }
    ...
    c3=(*(thePhaseFactors))[IndexPhaseFunction]->c3;
    c4=(*(thePhaseFactors))[IndexPhaseFunction]->c4;
    val=c0*std::sin(angle)*(c1*c2*(1+c3*std::pow(cos(angle),2.0))+(1-c1)*(1/(4*pi))*(1-c4*c4)/std::pow(1+c4*c4-2*c4*std::cos(angle),1.5));
  }
  return val;
}
...
// -----------------
//
G4double G4OpMie::GetMeanFreePath(const G4Track& aTrack,
				  G4double ,
				  G4ForceCondition* )
{
  const G4DynamicParticle* aParticle = aTrack.GetDynamicParticle();
