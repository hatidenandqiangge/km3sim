...
//     G4double thePos=(aStep->GetTrack()->GetPosition()-G4ThreeVector(10.0*m,10.0*m,19.0*km)).mag()/m;
...
//   }
  ...
  if(aStep->GetTrack()->GetParentID()==0){  //only the primary particle
    if(aStep->GetTrack()->GetDefinition() == G4MuonPlus::MuonPlusDefinition() || aStep->GetTrack()->GetDefinition() == G4MuonMinus::MuonMinusDefinition()){      
	...
	G4ThreeVector StartDirection=aStep->GetTrack()->GetVertexMomentumDirection();
	...
	if(iDist==iSlots)event_action->EnergyAtPosition.push_back(aStep->GetTrack()->GetKineticEnergy());
	else if(iDist>iSlots){
	  G4double LastEnergy=event_action->EnergyAtPosition[iSlots-1];
	  for(int ir=0 ; ir<iDist-iSlots ; ir++)event_action->EnergyAtPosition.push_back(LastEnergy);
	  event_action->EnergyAtPosition.push_back(aStep->GetTrack()->GetKineticEnergy());
	}
      }
#endif
      // here report the position of the muon when it stops
      if(aStep->GetTrack()->GetKineticEnergy()==0.0 && aStep->GetTrack()->GetTrackStatus()==fStopButAlive){
	...
	if(MuonSlot>=0){
      ...
      //////////////new/ here kill muons that have not enough energy to reach the can////////////////////////////
      ...
      if( (rxy2 > RRR2) || (x0[2]<myStDetector->bottomPosition) || (x0[2] > myStDetector->detectorMaxz) ){ //if it is not inside the can
	...
	if(dia >0){
	  ...
	  if(SideDistIn1 >0 && sidezIn1 > myStDetector->bottomPosition && sidezIn1 < myStDetector->detectorMaxz )SideDistIn=SideDistIn1;
	  if(SideDistIn2 >0 && sidezIn2 > myStDetector->bottomPosition && sidezIn2 < myStDetector->detectorMaxz && SideDistIn2<SideDistIn)SideDistIn=SideDistIn2;
	}
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      //new here we report the points when the muon goes in the detector (smaller can by 50 meters on the bottom and 150m on top and sides)//////////////////
	...
	G4double HorDistTop2 = PointOnTop[0]*PointOnTop[0] + PointOnTop[1]*PointOnTop[1];
	G4double HorDistBottom2 = PointOnBottom[0]*PointOnBottom[0] + PointOnBottom[1]*PointOnBottom[1];
	...
	if( (HorDistTop2 > RRR2) || (HorDistBottom2 > RRR2) ){
	  ...
	  if(dia <0){conti=false;}
	    ...
	    if( ((sidez1 < PointOnBottom[2]) && (sidez2 < PointOnBottom[2])) || ((sidez1 > PointOnTop[2]) && (sidez2 > PointOnTop[2])) ){conti=false;}
	    else if( ((sidez1 > PointOnBottom[2]) && (sidez2 > PointOnBottom[2])) && ((sidez1 < PointOnTop[2]) && (sidez2 < PointOnTop[2])) ){
	      if(sidez1 < sidez2){
		DistToTop=SideDist2;
	    ...
	    else if( (PointOnBottom[2]<sidez1) && (sidez1< PointOnTop[2]) ){
	...
	if(conti){
...
#endif
	  //before G4ThreeVector PointOnMiddle = PointOnTop - (PointOnTop.dot(PointOnBottom-PointOnTop)/(PointOnBottom-PointOnTop).mag2())*(PointOnBottom-PointOnTop);
	  ...
	  if( (DistToCenter<5.0*m) && (DistToCenter>-5.0*m)){
	    event_action->centerMomentum[MuonSlot] = aStep->GetTrack()->GetMomentum().mag();
	    event_action->centerPosition[MuonSlot] = x0;
	    event_action->centerTime[MuonSlot] = aStep->GetTrack()->GetGlobalTime();
	  }
	  ...
	  if( (DistToEnter<5.0*m) && (DistToEnter>-5.0*m)){
	    event_action->enterMomentum[MuonSlot] = aStep->GetTrack()->GetMomentum().mag();
	    event_action->enterPosition[MuonSlot] = x0;
	    event_action->enterTime[MuonSlot] = aStep->GetTrack()->GetGlobalTime();
	  }
	  ...
	  if( (DistToLeave<5.0*m) && (DistToLeave>-5.0*m)){
	    event_action->leaveMomentum[MuonSlot] = aStep->GetTrack()->GetMomentum().mag();
	    event_action->leavePosition[MuonSlot] = x0;
	    event_action->leaveTime[MuonSlot] = aStep->GetTrack()->GetGlobalTime();
	  }
	}
      }

      ////////here kill only muons that are leaving the can. All other particles are already killed if outside the can
      G4ThreeVector distanceV;
      distanceV=x0-myStDetector->detectorCenter;
      if( (x0[2]<myStDetector->bottomPosition) && (p0[2]<0) ){ //goes down while below the can
	aStep->GetTrack()->SetTrackStatus(fStopAndKill);
	return;
      }
      if( (x0[2]>myStDetector->detectorMaxz) && (p0[2]>0) ){ //goes up while above the can
	aStep->GetTrack()->SetTrackStatus(fStopAndKill);
	return;
      }
      direction=p0[0]*distanceV[0] + p0[1]*distanceV[1] ;
      distanceRho=sqrt(distanceV[0]*distanceV[0] + distanceV[1]*distanceV[1]);
      if( (distanceRho>myStDetector->detectorMaxRho) && (direction>0) ){ //goes away while outside the can
	aStep->GetTrack()->SetTrackStatus(fStopAndKill);
	return;
      }
