...
KM3MuonParam::KM3MuonParam() {
  FILE *infile;
  double energy, distance, prob0;
  int nbins;
  double tempoArray[10000];
  ...
  if ((infile = fopen("SimLengthProb", "r")) == NULL) {
    ...
    while (fscanf(infile, "%lf %lf %lf %d\n", &energy, &distance, &prob0,
      ...
      double numrejected = 0;
      int iterat = 1;
      double stopbin = energy * double(iterat + 1) / double(nbins);
      ...
      double nlastrej = tempoArray[iterat] * (stopbin - EnergyCutoff) *
                          double(nbins) / energy;
      ...
      if (prob0 < 0.999) {
        ...
        aPDFSList->StopFirstBin =
            double(int((EnergyCutoff / energy) * double(nbins)) + 1) * energy /
            double(nbins);
        aPDFSList->thePDF = new CLHEP::RandGeneral(tempoArray, nbins);
        thePDFS.push_back(aPDFSList);
        if (aPDFSList->LogEnergy < MinLogEnergy)
          MinLogEnergy = aPDFSList->LogEnergy;
        if (aPDFSList->LogEnergy > MaxLogEnergy)
          MaxLogEnergy = aPDFSList->LogEnergy;
      }
    }
  }
  fclose(infile);
...
void KM3MuonParam::AddMuon(double energy, double distance) {
  ...
  if (logenergy > MinLogEnergy && logenergy < MaxLogEnergy) {
    ...
    if (distance - thePDFS[ipthis]->Distance > 100.0 * m)
      aforMuon->iscapable = false;
...
void KM3MuonParam::Initialize(void) {
  ...
  for (size_t ip = 0; ip < theDistributions.size(); ip++) {
    if (theDistributions[ip]->iscapable &&
        theDistributions[ip]->thePDF == NULL) {
      isEventOK = false;
      break;
    }
  }
...
double KM3MuonParam::GetDistance(int idmuon) {
  return theDistributions[idmuon]->Distance;
}

double KM3MuonParam::GetEnergy(int idmuon) {
  if (!isEventOK)
    return pow(10.0, theDistributions[idmuon]->LogEnergy);
  else {
    if (G4UniformRand() < theDistributions[idmuon]->Prob0)
      return 0.0;
    else {
      double thefrac = theDistributions[idmuon]->thePDF->shoot();
      double theenergy =
          thefrac * pow(10.0, theDistributions[idmuon]->LogEnergy);
      if (theenergy < EnergyCutoff)
        theenergy = EnergyCutoff +
                    (theDistributions[idmuon]->StopFirstBin - EnergyCutoff) *
                        G4UniformRand();
      return theenergy;
    }
  }
...
bool KM3MuonParam::IsCapable(int idmuon) {
  return theDistributions[idmuon]->iscapable;
}

double KM3MuonParam::GetWeight(void) {
  double allprob0 = 1.0;
  for (size_t ip = 0; ip < theDistributions.size(); ip++)
