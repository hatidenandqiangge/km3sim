...
void HOURSevtWRITE::ReadEvent()
{
  ...
  if(UseEarthLepton)NumberOfParticles=evt->ndat("track_earthlepton");
  else NumberOfParticles=evt->ndat("track_in");
  int icount=0;
  for(int ip=0 ; ip<NumberOfParticles ; ip++){
    ...
    GetArgs(ParticleInfo,argnumber,args);
    if((int)args[9]>0){//do not count particles not within standard pdg coding
...
void HOURSevtWRITE::WriteEvent()
{
  evt->write(outfile);
}

void HOURSevtWRITE::AddHit(int id,int PMTid,double pe,double t,int trackid,int npepure,double ttpure,int creatorProcess)
{
  string dt("hit");
  char buffer[256];
  int Gid;
  if(trackid<=NumberOfParticles){
    ...
    trackid=ParticlesIdNumber[trackid-1];//convert from geant track id to input track id
  }
  else{
    Gid=6;
    trackid=999999;
  ...
  sprintf(buffer,"%8d %6d %6.2f %10.2f %4d %4d %3d %10.2f %4d",id,PMTid,pe,t,Gid,trackid,npepure,ttpure,creatorProcess);
  string dw(buffer);
  evt->taga(dt,dw);
}
					...
					double momx,double momy,double momz,double mom,double time)
{
  ...
  char buffer[256];
  tracknumber=ParticlesIdNumber[tracknumber-1];//convert from geant track id to input track id
  sprintf(buffer,"%4d %2d %8.2f %8.2f %8.2f %10.6f %10.6f %10.6f %12.6e %10.2f",
	  tracknumber,positionnumber,posx,posy,posz,momx,momy,momz,mom,time);
  string dw(buffer);
  evt->taga(dt,dw);
}
					...
					double posx,double posy,double posz,double time)
{
  ...
  char buffer[256];
  tracknumber=ParticlesIdNumber[tracknumber-1];//convert from geant track id to input track id
  sprintf(buffer,"%4d %2d %8.2f %8.2f %8.2f %10.2f",
	  tracknumber,positionnumber,posx,posy,posz,time);
  string dw(buffer);
  evt->taga(dt,dw);
}
					    ...
					    double energy, double time, int idPDG)
{
  ...
  parentID=ParticlesIdNumber[parentID-1];//convert from geant track id to input track id
  string dt("muon_decay");
  char buffer[256];
  sprintf(buffer,"%6d %6d %10.3f %10.3f %10.3f %12.8f %12.8f %12.8f %12.6f %10.2f %10d",trackID,parentID,
	  posx,posy,posz,dx,dy,dz,energy,time,idPDG);
  string dw(buffer);
  evt->taga(dt,dw);
}
...
void HOURSevtWRITE::AddMuonEnergyInfo(const vector<double> & info)
{
  ...
  for(int itag=0 ; itag<NumberOfTags ; itag++){
    ...
    int i=istart;
    if(nentries==10)sprintf(buffer,"%4d %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e",
			    itag,info[i],info[i+1],info[i+2],info[i+3],info[i+4],info[i+5],info[i+6],info[i+7],info[i+8],info[i+9]);
    else if(nentries==9)sprintf(buffer,"%4d %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e",
				itag,info[i],info[i+1],info[i+2],info[i+3],info[i+4],info[i+5],info[i+6],info[i+7],info[i+8]);
    else if(nentries==8)sprintf(buffer,"%4d %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e",
				itag,info[i],info[i+1],info[i+2],info[i+3],info[i+4],info[i+5],info[i+6],info[i+7]);
    else if(nentries==7)sprintf(buffer,"%4d %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e",
				itag,info[i],info[i+1],info[i+2],info[i+3],info[i+4],info[i+5],info[i+6]);
    else if(nentries==6)sprintf(buffer,"%4d %8.2e %8.2e %8.2e %8.2e %8.2e %8.2e",
				itag,info[i],info[i+1],info[i+2],info[i+3],info[i+4],info[i+5]);
    else if(nentries==5)sprintf(buffer,"%4d %8.2e %8.2e %8.2e %8.2e %8.2e",
				itag,info[i],info[i+1],info[i+2],info[i+3],info[i+4]);
    else if(nentries==4)sprintf(buffer,"%4d %8.2e %8.2e %8.2e %8.2e",
				itag,info[i],info[i+1],info[i+2],info[i+3]);
    else if(nentries==3)sprintf(buffer,"%4d %8.2e %8.2e %8.2e",
				itag,info[i],info[i+1],info[i+2]);
    else if(nentries==2)sprintf(buffer,"%4d %8.2e %8.2e",
				itag,info[i],info[i+1]);
    else if(nentries==1)sprintf(buffer,"%4d %8.2e",
				itag,info[i]);
    string dw(buffer);
    evt->taga(dt,dw);
  }
