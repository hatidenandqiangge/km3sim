...
{
  G4double b=sqrt(x*x+2*x*511.)*powf(1300.-x,2.0)*(x+511.); //see for example hyperphysics
  // "A simple relation for the Fermi function", eq 7
  G4double Z=20.0;
  G4double E=x+511.0;
  G4double P=sqrt(E*E-511.0*511.0);
  G4double eta=(Z/137.0)*E/P;
  G4double fermin=fabs(2.0*3.14159*eta/(1.0-exp(-2.0*3.14159*eta)));
  G4double w=x+511.0;
  G4double g=Z/137.0;
  G4double s=sqrt(1.0-powf(Z/137.0,2.0))-1.0;
  G4double fermi=fermin*powf(w*w*(1.0+4.0*g*g)-1.0,s);
  b=b*fermi;
  //half-life determination of K40 by LSC Malonda,Carles, Applied Radiation and Isotopes 56 (2002) 153-156
  w=x/511.+1.0;
  G4double wm=1300./511.+1.0;
  G4double p=sqrt(w*w-1.0);
  G4double q=wm-w;
  s=powf(p,6.0)+7*powf(p,4.0)*powf(q,2.0)+7*powf(p,2.0)*powf(q,4.0)+powf(q,6.0); 
  b=b*s;
  return b; 
}
#endif
...
{
  if(useHEPEvt && !useANTARESformat)HEPEvt = new G4HEPEvtInterface(filePythiaParticles);
#ifdef G4MYMUON_PARAMETERIZATION
  myMuonParam = new KM3MuonParam(); //for muon param vs distance
#endif
#ifndef G4MYFIT_PARAMETERIZATION
#ifndef G4MYEM_PARAMETERIZATION
#if (defined(G4MYHA_PARAMETERIZATION) && defined(G4MYHAMUONS_PARAMETERIZATION)) || !defined(G4MYHA_PARAMETERIZATION)
  if(useANTARESformat){
    ...
    infile=fopen(fileParticles,"r");  //it contains the number of events, the particle type, the vertex and momentum information
    G4double runtime;
    fscanf(infile,"%d %lf\n",&nevents,&runtime);
  }
#endif
...
#ifdef G4HADRONIC_COMPILE
#ifndef G4DISABLE_PARAMETRIZATION
  aHAVertexMuons = new HAVertexMuons("TheMuons_Hadron.dat","TheMuons_Hadron.index");
#endif
#endif
#endif
#endif
#endif
  if(outfile==NULL && !useANTARESformat)G4cout<<"ERROR OUTFILE\n"<<G4endl;
}
//primary particle generation. Single, multiple (many vertexes) particles and neutrino interaction events (single vertex) are supported
...
void KM3PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  G4double pxneu,pyneu,pzneu;  //neutrino momentum (GeV/c) if neutrino interaction
  G4double xx0,yy0,zz0;        //Vertex of injected or produced particles (in cm)
  G4double pxx0,pyy0,pzz0;     //Momentum of injected or produced particles (in GeV/c)
...
#endif
  if(ievent==1 && !useANTARESformat)fprintf(outfile,"%d\n",nevents);     //write the number of events
  if(!useHEPEvt){
    idtarget=0;                        //the target id is not relevant in case of injected particles.
    idneu=0;                           //neither is the neutrino id
    xneu=0.0;yneu=0.0;zneu=0.0;        //or the neutrino interaction vertex
    pxneu=0.0;pyneu=0.0;pzneu=0.0;     //or the neutrino momentum
    if(!useANTARESformat){
...
#ifdef G4MYMUON_PARAMETERIZATION
    //newcode for muon param vs distance // applicable only for simulation of atmospheric muons from EAS
    ...
    for(G4int ipart=0 ; ipart<numberofParticles ; ipart++){
      if(useANTARESformat){
	antaresHEPEvt->GetParticleInfo(idbeams[ipart],
				       xx0,yy0,zz0,pxx0,pyy0,pzz0,t0s[ipart]);
      }
      else fscanf(infile,"%d %lf %lf %lf %lf %lf %lf %lf\n",&idbeams[ipart],
		  &xx0,&yy0,&zz0,&pxx0,&pyy0,&pzz0,&t0s[ipart]);
      theMuonsPositions.push_back(G4ThreeVector(xx0*cm,yy0*cm,zz0*cm));
      theMuonsMomenta.push_back(G4ThreeVector(pxx0*GeV,pyy0*GeV,pzz0*GeV));
      ...
      G4double Ttop=(detectorMaxz - theMuonsPositions[ipart][2] )/theMuonsDirections[ipart][2];
      if(Ttop > 0){
	G4ThreeVector PointOnTop(distanceV[0]+Ttop*theMuonsDirections[ipart][0],
				 distanceV[1]+Ttop*theMuonsDirections[ipart][1],
				 distanceV[2]+Ttop*theMuonsDirections[ipart][2]);
	
	G4double dRhoTop = PointOnTop[0]*PointOnTop[0] + PointOnTop[1]*PointOnTop[1];
	if(dRhoTop < RRR2)distances[ipart]=Ttop;
	
	//next check if is going to hit the side of the detector
	G4double c=distanceV[0]*distanceV[0] + distanceV[1]*distanceV[1]-RRR2;
	if(c > 0.0 && distances[ipart]<0){
	  G4double a=theMuonsDirections[ipart][0]*theMuonsDirections[ipart][0] + theMuonsDirections[ipart][1]*theMuonsDirections[ipart][1];
	  G4double b=distanceV[0]*theMuonsDirections[ipart][0] + distanceV[1]*theMuonsDirections[ipart][1];
	  G4double dia=b*b - a*c;
	  if(dia >0){
	    dia=sqrt(dia);
	    G4double SideDist1= (-b-dia)/a;
	    G4double sidez1=theMuonsPositions[ipart][2]+SideDist1*theMuonsDirections[ipart][2];
	    if( (sidez1 > bottomPosition) && (sidez1 < detectorMaxz) && (SideDist1 >0) )
    ...
    if(distancemax < 0.0){ //no muon that moves to the detector. put a null muon
      numberofParticles=1;
      EventWeight=1.0;
      if(!useANTARESformat)fprintf(outfile,"%d %.6e\n",numberofParticles,EventWeight);
      G4PrimaryParticle* initialParticle = new G4PrimaryParticle(idbeams[0],0.0,0.0,0.01*GeV);
      G4PrimaryVertex* vertex = new G4PrimaryVertex(theMuonsPositions[0],t0s[0]);
      vertex->SetPrimary( initialParticle );
      anEvent->AddPrimaryVertex(vertex);
      G4cout<<"Generating vertex with x0= "<<theMuonsPositions[0][0]<<" y0= "<<theMuonsPositions[0][1]<<
	" z0= "<<theMuonsPositions[0][2]<<G4endl;
      G4cout<<"Generating null particle with px0= 0.0 py0= 0.0 pz0= 10.0MeV "<<ievent<<G4endl; 
      //write the beam and target ids (PDG codes), the vertex (cm), momentum (GeV/c)
      if(!useANTARESformat)fprintf(outfile,"%d %.6e %.6e %.6e 0.0 0.0 0.01 %.6e\n",idbeams[0],
	      theMuonsPositions[0][0]/cm,theMuonsPositions[0][1]/cm,theMuonsPositions[0][2]/cm,t0s[0]);
      if( (idbeams[0] == 13) || (idbeams[0] == -13) ){
	event_action->AddPrimaryNumber(1);
      }
    }
    else{ //there are some particles moving to the detector. Consider only the ones that move to.
      for(G4int ipart=0 ; ipart<numberofParticles ; ipart++){
	if(distances[ipart] >= 0.0)myMuonParam->AddMuon(energies[ipart],distances[ipart]);
      }
	...
	if(distances[ipart] >= 0.0){
      ...
      if(numcapable==0){ //no muon capable to reach the detector. put a null muon
	numberofParticles=1;
	EventWeight=1.0;
	if(!useANTARESformat)fprintf(outfile,"%d %.6e\n",numberofParticles,EventWeight);
	G4PrimaryParticle* initialParticle = new G4PrimaryParticle(idbeams[0],0.0,0.0,0.01*GeV);
	...
	if( (idbeams[0] == 13) || (idbeams[0] == -13) ){
      ...
      else{ // there are some muons capable to reach the detector. Consider only the ones that can.
	...
	for(G4int ipart=0 ; ipart<numberofParticleskeep ; ipart++){
	  if(distances[ipart] >= 0.0){
	    if(energies[ipart] > 0.0){
	      G4cout << "FERtest "<<distances[ipart]<<" "<<energies[ipart]<<G4endl;
	      theMuonsMomenta[ipart]=energies[ipart]*theMuonsDirections[ipart];
	      theMuonsPositions[ipart]+=distances[ipart]*theMuonsDirections[ipart];
	      t0s[ipart]+=distances[ipart]/c_light;
	      G4PrimaryParticle* initialParticle = new G4PrimaryParticle(idbeams[ipart],theMuonsMomenta[ipart][0],
									 theMuonsMomenta[ipart][1],theMuonsMomenta[ipart][2]);
	      G4PrimaryVertex* vertex = new G4PrimaryVertex(theMuonsPositions[ipart],t0s[ipart]);
	      vertex->SetPrimary( initialParticle );
	      anEvent->AddPrimaryVertex(vertex);
	      G4cout<<"Generating vertex with x0= "<<theMuonsPositions[ipart][0]<<" y0= "<<theMuonsPositions[ipart][1]<<
		" z0= "<<theMuonsPositions[ipart][2]<<G4endl;
	      G4cout<<"Generating particle with px0= "<<theMuonsMomenta[ipart][0]<<" py0= "<<theMuonsMomenta[ipart][1]<<
		" pz0= "<<theMuonsMomenta[ipart][2]<<"MeV "<<ievent<<G4endl; 
	      //write the beam and target ids (PDG codes), the vertex (cm), momentum (GeV/c)
	      if(!useANTARESformat)fprintf(outfile,"%d %.6e %.6e %.6e %.6e %.6e %.6e %.6e\n",idbeams[ipart],theMuonsPositions[ipart][0]/cm,
		      theMuonsPositions[ipart][1]/cm,theMuonsPositions[ipart][2]/cm,theMuonsMomenta[ipart][0]/GeV,theMuonsMomenta[ipart][1]/GeV,
    ...
    if(!useANTARESformat)fprintf(outfile,"%d %.6e\n",numberofParticles,EventWeight);
    for(G4int ipart=0 ; ipart<numberofParticles ; ipart++){
      //define the particle object and properties from the particle PDG code idbeam
      if(useANTARESformat){
	antaresHEPEvt->GetParticleInfo(idbeam,xx0,yy0,zz0,pxx0,pyy0,pzz0,t0);
      }
      else fscanf(infile,"%d %lf %lf %lf %lf %lf %lf %lf\n",&idbeam,&xx0,&yy0,&zz0,&pxx0,&pyy0,&pzz0,&t0);
      G4PrimaryParticle* initialParticle = new G4PrimaryParticle(idbeam,pxx0*GeV,pyy0*GeV,pzz0*GeV);
      G4PrimaryVertex* vertex = new G4PrimaryVertex(G4ThreeVector(xx0*cm,yy0*cm,zz0*cm),t0*ns);
      vertex->SetPrimary( initialParticle );
      anEvent->AddPrimaryVertex(vertex);
      G4cout<<"Generating vertex with x0="<<xx0<<" y0="<<yy0<<" z0="<<zz0<<G4endl;
      G4cout<<"Generating particle with px0="<<pxx0<<" py0="<<pyy0<<" pz0="<<pzz0<<" "<<ievent<<G4endl; 
      //write the beam and target ids (PDG codes), the vertex (cm), momentum (GeV/c)
      if(!useANTARESformat)fprintf(outfile,"%d %.6e %.6e %.6e %.6e %.6e %.6e %.6e\n",idbeam,xx0,yy0,zz0,pxx0,pyy0,pzz0,t0);
#ifndef G4MYHA_PARAMETERIZATION //newha
  ...
  if(!useHEPEvt){
    ...
    G4PrimaryParticle* initialParticle = new G4PrimaryParticle(idbeam,0.0,0.0,ParamEnergy);
    G4PrimaryVertex* vertex = new G4PrimaryVertex(G4ThreeVector(0.0,0.0,startz),0.0);
    vertex->SetPrimary( initialParticle );
    anEvent->AddPrimaryVertex(vertex);
    event_action->AddPrimaryNumber(1);
  ...
  if(!useHEPEvt){
    ...
    direction[1]=0.0;
    direction[2]=1.0;
    G4double zpos=3.46; //it is the shift for 100.0GeV pion and kaon zero long. see also the EM parametrization section in here
    G4ThreeVector thisPosition=position-zpos*m*direction;
    G4PrimaryVertex* vertex = new G4PrimaryVertex(thisPosition,0.0);
    numberofParticles=1;
    //type of incident particle for HA parametrization is input in the command line
    //    idbeam=211; //pion plus
    //    idbeam=321; //kaon plus
    //    idbeam=130; // kaon zero long
    //    idbeam=2112; //Neutron
    //    idbeam=2212; //Proton
    G4PrimaryParticle* initialParticle = new G4PrimaryParticle(idbeam,
								 ParamEnergy*direction[0],
								 ParamEnergy*direction[1],
  ...
  if(!useHEPEvt){
   ...
   static FILE* SNdata=NULL;
    ...
    //////////////////////
    numberofParticles=1;
    G4PrimaryParticle* initialParticle = new G4PrimaryParticle(-11,
							       ParamEnergy*direction[0],//momentum
							       ParamEnergy*direction[1],
							       ParamEnergy*direction[2]);
    vertex->SetPrimary( initialParticle );
    anEvent->AddPrimaryVertex(vertex);
    G4cout<<"Generating positron from SN event "<<ievent<<" with energy "<< PositronEnergy<<G4endl;
    //-----------------antonis-------------------------//
  }
#endif
  //laser beam
  ...
  if(!useHEPEvt){
    ///at first initialize the pointers to Rindex,Q_E, glass and gell transparencies////
    ...
    idbeam=Num_Laser_Photons; //Optical Photon . Here we put the number of photons for writing purposes on output file
    ...
    if(ievent==1){
      ...
      fscanf(LaserData,"%lf %lf %lf %lf %lf %lf\n",&LaserX,&LaserY,&LaserZ,&LaserDX,&LaserDY,&LaserDZ);
      fclose(LaserData);
      ...
      fprintf(outfile,"%.6e %.6e %.6e %.6e %.6e %.6e %.6e\n",LaserX,LaserY,LaserZ,LaserDX,LaserDY,LaserDZ,Wavelength_Laser_Photons/nm);
    }
    
    G4ParticleDefinition* OPDefinition = G4OpticalPhoton::OpticalPhotonDefinition();
    G4PrimaryVertex* vertex = new G4PrimaryVertex(position,0.0);
    numberofParticles=0;
    for(G4int ip=0 ; ip<Num_Laser_Photons ; ip++){  //Num_Laser_Photons is the number of photns before the relative QE and transparencies
      G4double qeProb=QECathod->Value(photonEnergy);
#ifdef G4MY_TRANSPARENCIES
      qeProb *= exp(-15.0/AbsBenth->Value(photonEnergy) - 10.0/AbsGell->Value(photonEnergy));
#endif
	...
	G4PrimaryParticle* initialParticle = new G4PrimaryParticle(OPDefinition,px,py,pz);
    ...
    anEvent->AddPrimaryVertex(vertex);
    G4cout<<"Generating one laser pulse "<<ievent<<" Number of photons "<<numberofParticles<<G4endl;	
  }
#endif
  //laser beam
#if defined(G4MYK40_PARAMETERIZATION) && !defined(G4MYSN_PARAMETERIZATION) && !defined(G4MYLASER_PARAMETERIZATION)
  if(!useHEPEvt){
    //-----------------antonis-------------------------//
	...
	idbeam=22;
      ...
      {
	idbeam=22;
        ParamEnergy=1.0*keV; //it should be zero, but it core dumps, so put a small energy (<Ch thres)
      }
    numberofParticles=1;
    ...
    vertex->SetPrimary( initialParticle );
    anEvent->AddPrimaryVertex(vertex);
    G4cout<<"Generating one K40 event "<<ievent<<" "<<idbeam<<" "<<ParamEnergy<<" "<<position/m<<" "<<direction<<G4endl;
  ...
  if(!useHEPEvt){
    ...
    if(ParamEnergy>0.0){
      //next we tranfer the particle backwards, in order the maximum (median) cherenkov emmision to happen at 0.0
      G4double enepos=log10(ParamEnergy/GeV);
      G4double zpos;
      if(idbeam==11 || idbeam==-11){
	if(enepos<0)zpos=0.90653+enepos*(0.96676+enepos*0.26983);
	else zpos=0.90132+enepos*0.81286;
      }
      else if(idbeam==22){
	if(enepos<0)zpos=1.2245+enepos*(0.82081+enepos*0.20987);
	else zpos=1.2088+enepos*0.786;
      }
      zpos*=m;
    ...
    else if(ParamEnergy == -1000.0){ //here is the parametrization for delta rays of low energy
      static G4double specin=-0.9832; //this is the (1+a) where a=-1.9832 the spectral index of muIoni generated electrons (kinetic energy)
      static G4double emin=pow(0.24,specin); //minimum kinetic energy 0.24MeV (threshold for cherenkov production
      static G4double emax=pow(31.6,specin); //maximum kinetic energy (above that em parametrization takes over)
      numberofParticles=10000;
      idbeam=11; //electron
      for(G4int ip=0 ; ip<numberofParticles ; ip++){
	G4double enekin=emin+G4UniformRand()*(emax-emin);
	enekin=pow(enekin,1.0/specin);
	G4double SpecMom=sqrt((enekin+0.511)*(enekin+0.511)-0.511*0.511); //0.511 is the electron mass in MeV
	G4double enelog=log10(enekin);
	G4double costheta=0.70351+enelog*(0.40702+enelog*(-0.12039+enelog*(-0.92576e-1+enelog*(0.64971e-1-0.98130e-2*enelog)))); //angle vs kinene
	if(costheta > 1.0)costheta=1.0;
	G4double sintheta=sqrt(1.0-costheta*costheta);
	G4double phi=2.0*pi*G4UniformRand();
	G4double cosphi=cos(phi);
	G4double sinphi=sin(phi);
	G4ThreeVector electronMomentum(SpecMom*sintheta*cosphi,SpecMom*sintheta*sinphi,SpecMom*costheta);
  ...
  ///end of initialization//////////////////////////////////////////////////////
      G4ParticleDefinition* OPDefinition = G4OpticalPhoton::OpticalPhotonDefinition();
      numberofParticles=0;
      idbeam=0; //Optical Photon . is not used , because it has not a pdgcoding in geant4
      for(G4int ip=0 ; ip<100000 ; ip++){  //100000 is the number of photns before the QE and transparencies
	

	//sample an optical photon//////////////////////////////////////////////
...
#ifdef G4MY_TRANSPARENCIES
	qeProb *= exp(-15.0/AbsBenth->Value(sampledMomentum) - 10.0/AbsGell->Value(sampledMomentum));
#endif
	if (G4UniformRand()<qeProb){
	  numberofParticles++;
	  ...
	  photonPolarization.rotateUz(direction);
	  //create the photon as primary
	  G4PrimaryParticle* initialParticle = new G4PrimaryParticle(OPDefinition,
								     photonMomentum[0],
								     photonMomentum[1],
								     photonMomentum[2]);
	  initialParticle->SetPolarization(photonPolarization[0],photonPolarization[1],photonPolarization[2]);
	  vertex->SetPrimary( initialParticle );
	}
	////////////////////////////////////////////////////////////////////////////
      }
    ...
    if(useANTARESformat){
      antaresHEPEvt->ReadEvent();
      antaresHEPEvt->GetNeutrinoInfo(idneu,idtarget,xneu,yneu,zneu,pxneu,pyneu,pzneu,t0);
    }
    else fscanf(infile,"%d %d %lf %lf %lf %lf %lf %lf\n",&idneu,&idtarget,&xneu,&yneu,&zneu,&pxneu,&pyneu,&pzneu);
    //Generate the Event (reads from Pythia output file)
    if(useANTARESformat){
      antaresHEPEvt->GeneratePrimaryVertex(anEvent);
    }
    else HEPEvt->GeneratePrimaryVertex(anEvent);
    ...
    numberofParticles=0;
    for(G4int iv=0 ; iv<numberofVertices ; iv++)numberofParticles+=anEvent->GetPrimaryVertex(iv)->GetNumberOfParticle();
    if(!useANTARESformat){
      fprintf(outfile,"%d %d %d\n",ievent,idneu,idtarget);  //write the event number (starting with 1) , the beam and target ids (PDG codes)
      fprintf(outfile,"%.6e %.6e %.6e %.6e %.6e %.6e\n",xneu,yneu,zneu,pxneu,pyneu,pzneu); //write the vertex (cm), momentum (GeV/c)
    }
    EventWeight=1.0;
    if(!useANTARESformat)fprintf(outfile,"%d %.6e\n",numberofParticles,EventWeight);
    G4int ipart=0;
    for(G4int iv=0 ; iv<numberofVertices ; iv++){
      for(G4int ip=0 ; ip<anEvent->GetPrimaryVertex(iv)->GetNumberOfParticle() ; ip++){
	...
	if(!useANTARESformat)fprintf(outfile,"%d %.6e %.6e %.6e %.6e %.6e %.6e %.6e\n",idbeam,xneu,yneu,zneu,pxx0,pyy0,pzz0,t0);  //vertex position is common in neutrino interaction
	if( (idbeam == 13) || (idbeam == -13) ){
	  event_action->AddPrimaryNumber(ipart+1);
	}
	ipart++;
    ...
    G4cout<<"Generating one neutrino event: evnum= "<<ievent<<" neutrinotype= "<<idneu<<" target= "<<idtarget<<" energy= "<<sqrt(pxneu*pxneu+pyneu*pyneu+pzneu*pzneu)<<" GeV"<<G4endl;
#ifdef G4HADRONIC_COMPILE
#ifndef G4DISABLE_PARAMETRIZATION
    //here only if hadronic interactions are used add muons from hadronic vertex
    //but first calculate the energy of hadrons
    direction = G4ThreeVector(0.0,0.0,0.0);
    G4double HadronicEnergy=0.0;
    for(G4int ipart=0 ; ipart<anEvent->GetPrimaryVertex(0)->GetNumberOfParticle() ; ipart++){ //only the first vertex which is neutrino vertex
      idbeam=fabs(anEvent->GetPrimaryVertex(0)->GetPrimary(ipart)->GetPDGcode());
      if(idbeam != 22 && idbeam !=13 && idbeam !=11 && idbeam !=15 && idbeam !=111){ //disregard em particles
	G4ThreeVector thisMomentum = anEvent->GetPrimaryVertex(0)->GetPrimary(ipart)->GetMomentum();
	G4double thisMass = anEvent->GetPrimaryVertex(0)->GetPrimary(ipart)->GetMass();
    ...
    if(HadronicEnergy > 0.0){
      ...
      HadronicEnergy /= GeV;
      G4int NumOfMuonsFromHAVertex=aHAVertexMuons->GetNumberOfMuons(HadronicEnergy);
	...
	xx0=aPosition[0]+xneu;yy0=aPosition[1]+yneu;zz0=aPosition[2]+zneu;
	pxx0=aMomentum[0];pyy0=aMomentum[1];pzz0=aMomentum[2];
	...
	anEvent->AddPrimaryVertex(vertex);      
      ...
      G4cout <<"Hadronic run. Event "<<ievent<< " with hadronic energy "<<HadronicEnergy<<" GeV"<<G4endl;
      G4cout <<"Number of Initial Particles "<<numberofParticles<<". Number of aditional muons "<<NumOfMuonsFromHAVertex<<G4endl;
...
//Here we do parametrization for hadronic interactions (primary particles that are considered are all except leptons, gamma and pion zero
//input files must have been generated from dif_cross application with hadron parametrization enabled
//that means that in the secondaries there are no leptons, gammas and pion zeros
//the output is just the energetic muons (Ek>1GeV)
    ...
    direction = direction.unit(); // this direction is used in the stacking action 
    HadronicEnergy -= 938.92*MeV;
    HadronicEnergy /= (float)GeV;
    outMuonHAFile->write((char*)&HadronicEnergy, sizeof (HadronicEnergy));
    G4cout <<"Hadronic parametrization. Event "<<ievent<< " with energy "<<HadronicEnergy<<" GeV"<<G4endl;
  }
#endif
#endif
#endif
