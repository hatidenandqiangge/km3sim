...
KM3EMDeltaFlux::KM3EMDeltaFlux(char* infileParam,G4double QEmax, G4double TotCathodArea)
{
  ...
  infile.seekg(std::streampos(634350756)); //it is 67540484*8+94026884 where the first number is size/energy,the second the number of energies and the third the size for direct flux
  ...
  for(G4int i=0 ; i<VertexDistanceBins ; i++){
    bool oka;
    KM3EMAngularFlux* aAngularFlux= new KM3EMAngularFlux(infile,oka,false); //false is for fine binning
...
void KM3EMDeltaFlux::FindBins(G4double MeanNumPhotons,G4double distancein, G4double anglein)
{
  G4int i;
