...
int main(int argc,char* argv[]) {
  ...
  G4cout << "The program has been compiled with hadronic physics and full particle list" << G4endl;
#else
  G4cout << "The program has been compiled with no hadronic physics and minimal particle list" << G4endl;
  G4cout << "Do not run this version for Pythia particle output. Only muons and electrons initial particles" << G4endl;
#endif

  if((argv[1][0]=='-')&&(argv[1][1]=='-')&&(argv[1][2]=='h')&&(argv[1][3]=='e')&&(argv[1][4]=='l')&&(argv[1][5]=='p')&&(argv[1][6]=='\0')){
    G4cout<<"argv[1] is a flag. If it is 'vrml' the detector is drawn in VRML format."<<G4endl;
    G4cout<<"                   If it is 'vrmle' also the event particle trajectories and hits are also shown"<<G4endl;
    ...
    G4cout<<"        the input is from neutrino interactions generator and" <<G4endl;
    G4cout<<"        argv[7] is the file name of the vertex and momentum neutrino information and" <<G4endl;
    G4cout<<"        argv[8] is the particle output of Pythia" <<G4endl;
    G4cout<<"else " <<G4endl;
    G4cout<<"        it is single (or multiple) particles (of any type) and" <<G4endl;
    G4cout<<"        argv[6] is the file name of the input data" <<G4endl;
    G4cout<<"endif" <<G4endl;
    return 0;
    // input from pythia or not the fileParticles must have the same format which is :
    // particle id, target id (meaningfull only for Pythia output), vertex (cm), momentum (GeV/c)
  }

  //find if visualization is requested
  G4bool visual;
  G4bool visuale;
  if((argv[1][0]=='v')&&(argv[1][1]=='r')&&(argv[1][2]=='m')&&(argv[1][3]=='l')&&(argv[1][4]=='\0')){
    visual=true;
    visuale=false;
  }
  else if((argv[1][0]=='v')&&(argv[1][1]=='r')&&(argv[1][2]=='m')&&(argv[1][3]=='l')&&(argv[1][4]=='e')&&(argv[1][5]=='\0')){
    visual=true;
    visuale=true;
  }
  else{
  ...
  G4int ParamNumber;
  G4int ParamParticle;
  if((argv[8][0]=='P')&&(argv[8][1]=='y')&&(argv[8][2]=='t')&&(argv[8][3]=='h')&&(argv[8][4]=='i')&&(argv[8][5]=='a')&&(argv[8][6]=='\0')){
    useHEPEvt=true;
    ...
    if(argv[10]==NULL){
      G4cout << "You must give a file with particle information from pythia" << G4endl;
      return 1;
    }
    filePythiaParticles=argv[10];
  }
  else if((argv[8][0]=='P')&&(argv[8][1]=='a')&&(argv[8][2]=='r')&&(argv[8][3]=='a')&&(argv[8][4]=='m')
      &&(argv[8][5]=='H')&&(argv[8][6]=='A')&&(argv[8][7]=='\0')){
    useHEPEvt=false;
    useANTARESformat=false;
    if(argv[9]==NULL){
    ...
    if(argv[12]==NULL){
      G4cout << "You must give the PDG code of the particle for Param" << G4endl;
      return 1;
    }
    else{
      ...
      &&(argv[8][5]=='H')&&(argv[8][6]=='A')&&(argv[8][7]=='M')&&(argv[8][8]=='u')&&(argv[8][9]=='o')
      &&(argv[8][10]=='n')&&(argv[8][11]=='s')&&(argv[8][12]=='\0')){
    useHEPEvt=true;
    useANTARESformat=false;
    if(argv[9]==NULL){
      G4cout << "You must give an input file" << G4endl;
    ...
    filePythiaParticles=argv[10];
    if(argv[11]==NULL){
      G4cout << "You must give a file to write high energy muon information for HA parameterization" << G4endl;
      return 1;
    }
    fileParamHAmuons=argv[11]; //this is fed to stacking and generator
    if(argv[12]==NULL){
      G4cout << "You must give a file to write photon information for HA parameterization" << G4endl;
      return 1;
    }
    else{
      if((outfilePar=fopen(argv[12],"w"))==NULL){ //this works the same way as in ParamEM
        printf ("Error open output Param photon file for Hadronic parameterization\n");
        return 1;
      }
    }
  }
  else if((argv[8][0]=='P')&&(argv[8][1]=='a')&&(argv[8][2]=='r')&&(argv[8][3]=='a')&&(argv[8][4]=='m')
      &&(argv[8][5]=='F')&&(argv[8][6]=='i')&&(argv[8][7]=='t')&&(argv[8][8]=='\0')){
    useHEPEvt=false;
    useANTARESformat=false;
    if(argv[9]==NULL){
      G4cout << "You must give an output param file" << G4endl;
    ...
    if(argv[11]==NULL){
      ...
      &&(argv[8][5]=='E')&&(argv[8][6]=='M')&&(argv[8][7]=='\0')){
    useHEPEvt=false;
    useANTARESformat=false;
    if(argv[9]==NULL){
      G4cout << "You must give an output param file" << G4endl;
  ...
  if(fileParamHAmuons==NULL){
    G4cout << "G4MYHA_PARAMETERIZATION preprocessor flag must be combined with ParamHA 7th command line argument" << G4endl;
    return 1;
  }
#endif
  //--------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  G4TrajectoryDrawByParticleID* model = new G4TrajectoryDrawByParticleID("KM3TrajectoryModel");
  ...
  G4TrajectoryParticleFilter* modelFilter = new G4TrajectoryParticleFilter("KM3TrajectoryFilterModel");
  ...
  myGeneratorAction->myTracking=myTracking;    //link between generator and tracking (to provide number of initial particles to trackingAction
...
#if !defined(G4MYEM_PARAMETERIZATION) && !defined(G4MYHA_PARAMETERIZATION) //newha
  myGeneratorAction->event_action=event_action;  //generator knows event to set the number of initial particles
#endif
#ifdef G4MYFIT_PARAMETERIZATION
  Mydet->event_action=event_action; //KM3SD must know the momentum of the particle when crosses the center of the detector
#endif
#if defined(G4MYEM_PARAMETERIZATION) || defined(G4MYHA_PARAMETERIZATION) //newha
  event_action->MyGenerator=myGeneratorAction; //event action knows generator for EM param purposes
...
#ifdef G4MYHAMUONS_PARAMETERIZATION
  std::ofstream* outMuonHAFile = new std::ofstream(fileParamHAmuons, std::ios::out | std::ios::binary);
  ...
  UI->ApplyCommand("/process/inactivate G4FastSimulationManagerProcess");G4cout << "the shower parametrization not used" << G4endl;
#endif
...
#if (defined(G4MYEM_PARAMETERIZATION) && !defined(G4MYK40_PARAMETERIZATION)) || defined(G4MYHA_PARAMETERIZATION)
  for(G4int i=0 ; i<VertexDistAngleBins ; i++){
    fprintf(outfilePar,"%.20Le %.20Le %.20Le\n",(*myCumNorma)[i],(*myCumPhotons)[i],(*myCumPhotonsRms)[i]);
    G4int kstart=i*OMSolidAngleBins;
    G4int kstop=kstart+OMSolidAngleBins;
    for(G4int k=kstart ; k<kstop ; k++)fprintf(outfilePar,"%.20Le %.20Le %.20Le\n",
        (*myPhotonsTh2)[k],
        (*myPhotonsTh3)[k],
        (*myPhotonsTh2Th3Num)[k]);
    G4int jstart=i*TimeSolidAngleBins*TimeBins;
    G4int jstop=jstart+TimeSolidAngleBins*TimeBins;
    for(G4int j=jstart ; j<jstop ; j++)fprintf(outfilePar,"%.20Le\n",(*myPhotonsTime)[j]);
  }
#endif
  ...
  // job termination
  if(!useANTARESformat)fclose(savefile);
  else delete TheEVTtoWrite;
#ifdef G4MYFIT_PARAMETERIZATION
  fclose(outfilePar);
#endif
#if defined(G4MYEM_PARAMETERIZATION) || defined(G4MYHA_PARAMETERIZATION) //newha
