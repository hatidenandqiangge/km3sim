...
class KM3TrackInformation : public G4VUserTrackInformation 
{
  ...
  public:
    inline G4String GetOriginalTrackCreatorProcess() const {return originalTrackCreatorProcess;}
    inline G4String GetOriginalParticleName() const {return originalparticleName;}
    ...
    inline G4bool   GetEmittedAsScattered() const {return EmittedAsScattered;} //newmie
#ifdef G4MYLASER_PARAMETERIZATION
  inline void KeepScatteringPosition(const G4ThreeVector& ScatteringPosition, double ScatteringAngle) {
    ScatteringPositions->push_back(ScatteringPosition);
    ScatteringAngles->push_back(ScatteringAngle);
  };
  inline G4int GetNumberOfScatters(void) {return ScatteringPositions->size();};
  inline G4ThreeVector GetScatteringPosition(G4int is) {return (*ScatteringPositions)[is];};
  inline G4double GetScatteringAngle(G4int is) {return (*ScatteringAngles)[is];};
#endif
};
