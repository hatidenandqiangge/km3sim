...
public:
  ...
  //short  void InsertExternalHit(G4int ic,G4double time,G4int originalInfo,G4int angleDirection,G4int angleIncident);
...
private:
  ...
  G4double TResidual(G4double , const G4ThreeVector&, const G4ThreeVector&, const G4ThreeVector&);
  void clear();
  void PrintAll();
  void QuickSort(G4int shorttype, std::vector<KM3Hit*>* theCollectionVector,G4int top, G4int bottom);
  G4int partition_CathodId(std::vector<KM3Hit*>* theCollectionVector,G4int top, G4int bottom);
  G4int partition_Time(std::vector<KM3Hit*>* theCollectionVector,G4int top, G4int bottom);
  G4bool AcceptAngle(G4double cosangle,G4double CathodRadius, G4double CathodHeight, bool);
